 ...
 */
    ...
    /**
     * 平台展销：检查上传图片指定的图片类型和分类是否正确
     */
    public function setImageTemaiCatId($imageCatId, $imageType)
    {
        if( !$this->userId  )
        {
            throw new \LogicException(app::get('image')->_('上传失败，平台展销会员ID不存在'));
        }
    ...
    public function setUploadUserId($userId = null)
    {
        $this->userId = $userId;
    }
    ...
    */
    ...
    public function companyStore( $fileObject, $from, $imageType, $test=false)
    {
        $fileObject = $this->__preFileObject( $fileObject );
        ...
        $accountData = $this->__companyImageAttach($from, $test);
        $params['target_id'] = $accountData['target_id'];
        $params['target_type'] = $accountData['target_type'];
        $params['disabled'] = 0;
     ...
     * @param string $from 上传图片用户类型
     */
    private function __companyImageAttach($from=false, $test=false)
    {
        if( $from == 'shop' )
        {
