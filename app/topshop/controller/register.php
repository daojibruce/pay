  ...
  //}
    ...
    public function enterProcessCompanyInfo()
    {
        ...
        if (!$companyInfo) {
            ...
            $company_area=$Info['company_area'] ;
            if( $company_area )
            {
                $area = '';
                ...
                $companyInfo['shop_info']['company_area'] = $area;
            }
            $companyInfo['shop_info']['company_addr'] = $Info['company_addr'];
            //$companyInfo['shop_info']['license_num'] = $Info['tax_id'];
            $companyInfo['shop_info']['enroll_capital'] = $Info['registered_capital'];
            $companyInfo['shop_info']['company_contacts'] = $Info['company_contact'];
            $companyInfo['shop_info']['company_phone'] = $Info['company_contactPhone'];
            $companyInfo['company_name'] = $Info['company_name'];
            $companyInfo['shop_info']['bank_user_name'] = $Info['company_name'];
            $companyInfo['shop_info']['license_indate'] = $Info['valid_time'];
        }

        $companyInfo['shop_info']['establish_date'] = $this->date2time($companyInfo['shop_info']['establish_date']);
        /*if ($companyInfo['shop_info']['license_indate'] == '长期有效') {
            unset($companyInfo['shop_info']['license_indate']);
            $companyInfo['shop_info']['license_indate_long'] = 1;
        } else {
            $companyInfo['shop_info']['license_indate'] = $this->date2time($companyInfo['shop_info']['license_indate']);
        }*/
        if ($companyInfo['shop_info']['license_indate'] == '长期有效') {
            $companyInfo['shop_info']['license_indate'] = 9999999999;
        } else {
            $companyInfo['shop_info']['license_indate'] = strtotime($companyInfo['shop_info']['license_indate']);
        }
//dd($companyInfo);
        $pagedata = $companyInfo;
        $pagedata['categories'] = app::get('syscategory')->model('cat')->getList('cat_id,cat_name',['parent_id' => 0]);
    ...
    public function enterProcessCompanyInfoAction()
    {
        ...
        $validator = validator::make ([
            ...
            //'shop_info.license_img' => app::get('topshop')->_('请上传企业授权书'),
        ...
        /*if ($companyInfo['shop_info']['license_indate']) {
            $license_indate = strtotime($companyInfo['shop_info']['license_indate']);
            if (time() > $license_indate) {
                return $this->splash('error',null,'营业期限时间不能小于当前时间！',true);
            }
        } elseif (!$companyInfo['shop_info']['license_indate_long']) {
            return $this->splash('error',null,'请填写营业期限',true);
        }*/
    ...
    public function enterProcessShopInfo()
    {
      ...
      $lv1Catlists = app::get('topshop')->rpcCall('category.cat.get.info',array('fields'=>'cat_id,cat_name','parent_id'=>0,'level'=>1));
        ...
        $pagedata['loginFlag'] = pamAccount::getAccountId() ?  true : false;
        if ($pagedata['brand_cat_id']) {
            $catInfo = app::get('syscategory')->model('cat')->getRow('cat_name',['cat_id' => $pagedata['brand_cat_id']]);
            $pagedata['new_cat_name'] = $catInfo['cat_name'];
        }
    ...
    {
        $shopInfo = input::get();
        if (!$shopInfo['brand_cat_id']) {
            unset($shopInfo['brand_cat_id']);
            unset($shopInfo['new_brand']);
            unset($shopInfo['new_brand_alias']);
        }
        ...
        else
        {
            ...
            $userId = userAuth::id();
            $Info = app::get('sysuser')->model('user_company')->getRow('*',['user_id' => $userId]);
            $post['shop']['shop_addr'] = $Info['company_addr'];
    ...
    private function __checkpost(&$postdata)
    {
        ...
        /*if ($postdata['shop_info']['license_indate_long']) {
            $postdata['shop_info']['license_indate'] = 9999999999;
            unset($postdata['shop_info']['license_indate_long']);
        } else {
    ...
    private function __checkPostImg($shopInfo,$shopType)
    {
        ...
        $imgInfo = [
                ...
                //'license_img' => app::get('topshop')->_("请上传企业授权书"),
