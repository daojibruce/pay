 ...
 */
final class ectools_payment_plugin_upacpb2b extends ectools_payment_app implements ectools_interface_payment_app {

    /**
     * @var string 支付方式名称
     */
    public $name = '中国银联网关企业支付(pc)';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = 'upacpb2b';
     /**
     * @var string 支付方式key
     */
    public $app_key = 'upacpb2b';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'upacpb2b';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '中国银联网关企业支付(pc)';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
    ...
    public $version = '5.1.0';		//版本号
	private $_encoding = 'utf-8';	//编码
	private $_channelType = '07';	//07PC, 08WAP
	private $_currencyCode = '156';	//货币类型
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_upacpb2b', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_upacpb2b', 'callback');
        ...
        // 退款异步返回地址
        $this->notify_url_refund = kernel::openapi_url('openapi.ectools_payment/parse/' . $this->app->app_id . '/ectools_payment_plugin_upacpb2b', 'refundcallback');
        if (preg_match("/^(http):\/\/?([^\/]+)/i", $this->notify_url, $matches))
        {
    ...
    public function admin_intro(){
        return '中国银联企业支付（UnionPay）是银联电子支付服务有限公司主要从事以互联网等新兴渠道为基础的网上支付。（适用于中国银联的企业用户）';
    }
    ...
    public function setting(){
        return array(
			...
			'gateway'=>array(
                'title'=>app::get('ectools')->_('支付网关url设置'),
                'type'=>'string',
				'length' => '100',
                'validate_type' => 'required',
            ),
    ...
    public function intro(){
        return '中国银联企业支付（UnionPay）是银联电子支付服务有限公司主要从事以互联网B2B等新兴渠道为基础的网上支付。';
    }
    ...
    public function dopay($payment){
        ...
        $cert_dir = DATA_DIR . '/cert/payment_plugin_upacpb2b/';
		...
		//由证书获取相关参数
		$this->__init_cert($sdk_sign_cert_path, $sdk_sign_cert_pwd);

        $params = array(
			'version' => $this->version,       //版本号
			'encoding' => $this->_encoding,    //编码方式
			...
			'signMethod' => '01',       //签名方法
			'channelType' => $this->_channelType,   //渠道类型，07-PC，08-手机
			'accessType' => '0',        //接入类型
			'currencyCode' => $this->_currencyCode,    //交易币种
			'merId' => $mer_id,               //商户代码，请改自己的测试商户号
			'orderId' => $payment['payment_id'],    //商户订单号
			'txnTime' => date('YmdHis'),      //订单发送时间
			'txnAmt' => number_format($payment['cur_money'] * 100, 0, '.', ''), //交易金额，单位分
			//'defaultPayType' => '0001', //默认支付方式
			//'orderDesc' => '订单描述',  //订单描述，网关支付和wap支付暂时不起作用
			//'reqReserved' => base64_encode('透传信息'), //请求方保留域，透传字段，查询、通知、对账文件中均会原样出现
        );//dump($params);exit;
        $this->sign ( $params, $sdk_sign_cert_path, $sdk_sign_cert_pwd);
        $front_uri = $this->getConf('gateway', __CLASS__);
    ...
    public function sign(&$params, $cert_path, $cert_pwd) {
        ...
        // 转换成key=val&串
        $params_str = $this->coverParamsToString ($params, true, false);
		//ksort($params);$params_str = http_build_query($params);
		$params_sha256x16 = hash('sha256', $params_str);

		// 签名
		$result = openssl_sign ( $params_sha256x16, $signature, $this->key, 'sha256');
		if ($result) {
	 ...
	 * @return string
	 */
	public function coverParamsToString($para, $sort, $encode) {
    ...
    private function __init_cert($cert_path, $cert_pwd) {
        $pkcs12 = file_get_contents ( $cert_path );
		if(!$pkcs12) exit('证书读取失败！');

        openssl_pkcs12_read ( $pkcs12, $certs, $cert_pwd );
        $this->cert = $certs['cert'];

        openssl_x509_read ($this->cert);
        $certdata = openssl_x509_parse ($this->cert);
    ...
    public function create_html($params, $submit_url, $target='')
    {
        ...
        $sHtml = $sHtml . "<input type='submit' value='submitForm' style='display:none;'></form>";
        $sHtml .= "<script>document.forms['applyForm'].submit();</script>";
        return $sHtml;
    }
