...
class topc_ctl_cart extends topc_controller {
    ...
    public function index()
    {
        ...
        foreach ($pagedata['aCart'] as $k=>$v) {
            ...
            // 获取店铺子域名
            $pagedata['aCart'][$k]['subdomain'] = app::get('topc')->rpcCall('shop.subdomain.get',array('shop_id'=>$v['shop_id']))['subdomain'];
            ...
            if(userAuth::check())
            {
                ...
                {
                    $pagedata['aCart'][$k]['hasCoupon'] = 1;
                }
            }
            foreach ($v['object'] as $key => $val) {
                $info = app::get('sysitem')->model('sku')->getRow('spec_code',['sku_id' => $val['sku_id']]);
                $pagedata['aCart'][$k]['object'][$key]['spec_code'] = $info['spec_code'];
            }
        }
        //重新组织数据，同一个item_id放在一起
        $pagedata = $this->reOrganize($pagedata);
    ...
    public function reOrganize($pagedata)
    {
        //echo "<pre>";print_r($pagedata);exit;
        foreach ($pagedata['aCart'] as $shopId => $shopGoods) {
            $reData = [];
            foreach ($shopGoods['object'] as $cart_id => $sku) {
                $reData[$sku['item_id']][] = $sku;
            }
            $pagedata['aCart'][$shopId]['object'] = $reData;
        }
        //检查是否全部选中
        $selectShops = [];
        $selectItems = [];
        $selectAll = 0;
        $countShops = count($pagedata['aCart']);
        foreach ($pagedata['aCart'] as $shopId => $shopGoods) {
            $countItems = count($shopGoods['object']);
            foreach ($shopGoods['object'] as $item_id => $item) {
                $countSkus = count($item);
                foreach ($item as $index => $sku) {
                    if ($sku['is_checked'] == '1') $countSkus -= 1;
                }
                if ($countSkus == '0') $selectItems[$shopId][$item_id] = 1;
            }
            if ($countItems == count($selectItems[$shopId])) $selectShops[$shopId] = 1;
        }
        if ($countShops == count($selectShops)) $selectAll = 1;

        $pagedata['selectItems'] = $selectItems;
        $pagedata['selectShops'] = $selectShops;
        $pagedata['selectAll'] = $selectAll;
    ...
    public function add()
    {
        ...
        $params['is_temai'] = input::get('is_temai');
        ...
        if( $params['obj_type']=='item')
        {
            ...
            if ($params['mode'] == 'fastbuy') {
                $params['ftime'] = time();
                foreach (input::get('item') as $key => $sku) {
                    $quantity = $sku['quantity'];
                    if ($quantity == 0) continue;
                    $flag = 0;
                    $params['quantity'] = $quantity ? $quantity : 1; //购买数量，如果已有购买则累加
                    $params['sku_id'] = intval($sku['sku_id']);
                    try
                    {
                        $data = kernel::single('topc_cart')->addCart($params);
            ...
            if ($flag) {
                $msg = app::get('topc')->_('商品数量为0，请添加数量！');
                return $this->splash('error',null,$msg,true);
            }
        }
    ...
    public function checkout()
    {
        ...
        $rs = userAuth::getUserRoles($params['user_id']);
        if(! isset($rs['roles'][SYS_USER_ROLE_COMPANY]['name'])){
            $isCompanyMember = false;
        }
        $pagedata['isCompanyMember'] = $isCompanyMember;
        ...
        foreach ($cartInfo['resultCartData'] as $k => $v) {
            foreach ($v['object'] as $key => $val) {
                $info = app::get('sysitem')->model('sku')->getRow('spec_code',['sku_id' => $val['sku_id']]);
                $cartInfo['resultCartData'][$k]['object'][$key]['spec_code'] = $info['spec_code'];
            }
        }
        ...
        foreach($cartInfo['resultCartData'] as $key=>&$val)
        {
            ...
            // 获取店铺子域名
            if($key != 'temai') $val['subdomain'] = app::get('topc')->rpcCall('shop.subdomain.get',array('shop_id'=>$val['shop_id']))['subdomain'];
        ...
        //重新组织数据结构
        $pagedata = $this->reCheckout($pagedata);
//echo "<pre>";print_r($pagedata);exit;
        return $this->page('topc/checkout/index.html', $pagedata);
    }

    public function reCheckout($pagedata)
    {
        //echo "<pre>";print_r($pagedata);exit;
        foreach ($pagedata['cartInfo']['resultCartData'] as $shopId => $shopGoods) {
            $reData = [];
            foreach ($shopGoods['object'] as $cart_id => $sku) {
                $reData[$sku['item_id']][] = $sku;
            }
            $pagedata['cartInfo']['resultCartData'][$shopId]['object'] = $reData;
        }
        //echo "<pre>";print_r($pagedata);exit;
        return $pagedata;
    }
    ...
    public function total()
    {
        ...
        foreach ($cartInfo['resultCartData'] as $shop_id => $tval) {
            ...
            if('temai' === $shop_id){
                $totalParams['shipping_type'] = $postData['shipping']['temai']['shipping_type'];
            }
