...
class topc_ctl_item extends topc_controller {
    ...
    public function index()
    {
        ...
        // 检测是否是平台展销商品并做价格和库存处理
        //$detailData = kernel::single('topc_temai_check')->reloadData($detailData);
        ...
        //如果是平台展销，未登录无法查看
        if ($detailData['is_temai'] && !$pagedata['user_id']) {
        ...
        //平台展销信息改造
        if (!$detailData['is_temai'] && $detailData['shop_id'] == '0') {
            return redirect::action('topc_ctl_default@index');
        }
        if(!$detailData['is_temai']) {
            //获取商品的促销信息
        ...
        //获取用户IP获取位置
        $pagedata['customAddr'] = $this->customAddr();

        //商品推荐
        $pagedata['itemRecommend'] = $this->itemRecommend($pagedata['item']['cat_id']);

        //商品浏览历史
        $pagedata['itemBrowserHistory'] = $this->itemBrowserHistoryGet();

        //获取该商品成交量
        $sql = "SELECT SUM(num) as total FROM systrade_order WHERE item_id=".$itemId." GROUP BY item_id";
        $itemHistoryBuy = app::get('base')->database()->executeQuery($sql)->fetchAll();
        $pagedata['itemHistoryBuy'] = $itemHistoryBuy['0']['total'];
        ...
        if(!$detailData['is_temai']) {
            //店铺热销排行
            $pagedata['itemHot'] = $this->itemHot($pagedata['item']['shop_id']);

            // 获取店铺子域名
            $pagedata['subdomain'] = app::get('topc')->rpcCall('shop.subdomain.get', array('shop_id' => $pagedata['shop']['shop_id']))['subdomain'];
            $condition = unserialize($pagedata['item']['step_price']);
            if (!$condition) {
                $pagedata['no_step_price'] = 1;
            } else {
                $arrSort = [];
                foreach ($condition as $uniqid => $row) {
                    foreach ($row as $key => $value) {
                        $arrSort[$key][$uniqid] = $value;
                    }
                }
                array_multisort($arrSort['quantity'], constant('SORT_ASC'), $condition);
                $pagedata['item']['step_price'] = $condition;
            }
        }

        //平台展销商品 特殊处理..
        if($detailData['is_temai']) {
            $pagedata['temai_info'] = $detailData['temai_info'];
            //是否可购买
            $pagedata['item']['is_buy'] = false;
            if($pagedata['temai_info']['state'] == 1 && $pagedata['temai_info']['store'] > 0) {
                $pagedata['item']['is_buy'] = true;
            }

            return $this->page('topc/temai/item/index.html', $pagedata);
        }
//echo "<Pre>";print_r($pagedata);exit;
        return $this->page('topc/items/index.html', $pagedata);
    }

    public function customAddr()
    {
        //调用淘宝接口判断用户位置
        $ip = $_SERVER['REMOTE_ADDR'];
        $url="http://ip.taobao.com/service/getIpInfo.php?ip=".$ip;
        $ipinfo=json_decode(file_get_contents($url));
        if($ipinfo->code=='1' || !$ipinfo->region) {
            return '无法获取当前位置';
        } else {
    ...
    public function itemHot($shop_id)
    {
        //获取店铺排行
        $sql = "SELECT SUM(num) as total, item_id, title, pic_path, price FROM systrade_order WHERE shop_id=".$shop_id." GROUP BY item_id ORDER BY total DESC LIMIT 0,5";
        $itemList = app::get('base')->database()->executeQuery($sql)->fetchAll();
        return $itemList;
    }

    public function itemRecommend($cat_id)
    {
        //获取推荐商品
        $itemList = app::get('sysitem')->model('item')->getList('item_id, title, price, image_default_id',['cat_id' => $cat_id,'disabled' => '0']);
        if ($itemList) {
            shuffle($itemList);
            $returns = array_slice($itemList, 0, 4);
        } else {
            $returns = array();
        }
    ...
    public function itemBrowserHistoryGet()
    {
        ...
        $data = app::get('topc')->rpcCall('item.list.get',['item_id'=>$itemIds , 'item_disabled' => '0', 'fields'=>$fields]);
    ...
    public function getStepPrice()
    {
        $input = input::get();
        if (!$input['sku_id']) {
            return response::json(['flag' => '1']);exit;
        }
        $skuInfo = app::get('sysitem')->model('sku')->getRow('step_price',['sku_id' => $input['sku_id']]);
        $condition = unserialize($skuInfo['step_price']);
        if (!$condition) {
            return response::json(['flag' => '1']);exit;
        }
        $arrSort = [];
        ...
        array_multisort($arrSort['quantity'], constant('SORT_DESC'), $condition);
        foreach ($condition as $k => $v) {
            if ($input['quantity'] >= $v['quantity']) {
                return response::json(['price' => $v['price']]);exit;
            }
        }
        return response::json(['flag' => '1']);exit;
    }
