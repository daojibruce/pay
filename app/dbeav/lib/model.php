...
class dbeav_model extends base_db_model
{
    ...
    function getListKeyField($cols='*', $filter=array() , $fieldName = null, $offset=0, $limit=-1, $orderBy=null)
    {
        if(! $fieldName){
            $fieldName = $this->idColumn;
        }
        $data = $this->getList($cols='*', $filter=array() , $offset=0, $limit=-1, $orderBy=null);
        $this->tidy_data($data, $cols);
        $keyData = array();
        foreach($data as $key => $arr){
            if(isset($arr[$fieldName])){
                $newKey = $arr[$fieldName];
                $keyData[$newKey] = $arr;
            }
        }
