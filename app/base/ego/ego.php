...
class omsapi_controller
{
    ...
    public function process()
    {
        ...
        try {
            ...
            if (!$version) {
                throw new RuntimeException(app::get('oms')->_('系统参数：版本号必填'), '10001');
            }
    ...
    {
        $method = trim($method);
        $omsapi = config::get('omsapi.routes.' . $version);

        if (!$omsapi) {
            throw new RuntimeException('该版本号不存在API', 10002);
        }

        if (!in_array($method, array_keys($omsapi))) {
            throw new RuntimeException('找不到API:' . $method);
        }

        $this->isCheckAccessToken = $omsapi[$method]['auth'] ? true : false;
        list($class, $fun) = $this->parseClassCallable($omsapi[$method]['uses']);
        $objclass = new $class();
    ...
    public function checkAccessToken($accessToken)
    {
        $userId = kernel::single('omsapi_token')->check($accessToken);
...
function ecos_bbcver()
{
	...
	if (defined('CUSTOM_CORE_DIR')) {
		if ($zendinfo['Product-Name'] != 'ShopEx-ONex-B2b2c-Source') {
	...
	switch ($techEdition) {
	case 'ALL':
		return array('topapi', 'topc', 'topwap');
	case 'NOPC':
		return array('topapi', 'topwap');
	case 'NOWAP':
		return array('topapi', 'topc');
	case 'NOAPP':
		return array('topc', 'topwap');
	case 'APP':
		return array('topapi');
	case 'PC':
		return array('topc');
...
function ecos_license_allow_to_topc()
{
	...
	if (!in_array('topc', $powers)) {
		exit(base64_decode('55Sx5LqO54mI5pys5Y+X6ZmQ77yM5oKo5peg5rOV6K6/6ZeuUEPpobXpnaLjgILlpoLmnInnlpHpl67vvIzor7fogZTns7vljYPljKDlrqLmnI3kurrlkZjjgII='));
	}
}
...
function ecos_get_code_license_info()
{
	...
	if (defined('GODMODE') && (GODMODE == 'ShopexBBCb90e1fd955f5127caba63169')) {
		$zl = array('Product-Name' => 'ShopEx-ONex-B2b2c-Source', 'Registered-To' => 'Qianjiang BPB Team', 'Max-Concurrent' => 'no', 'Lease-Expiration' => 'no', 'Wait-For-Lease' => 'no', 'Expires' => 'Never', 'Hardware-Locked' => 'No', 'Edition' => 'ENTERPRISE-APP', 'Tech-Edition' => 'NOPC', 'Host-Name' => 'NOLIMIT', 'Produced-By' => '千匠');
		return $zl;
	}
