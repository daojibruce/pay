...
class sysitem_ctl_admin_item extends desktop_controller{
    ...
    public function editWeight($item_id)
    {
        $pagedata['item'] = app::get('sysitem')->model('item')->getRow('title,item_id,order_weight',['item_id' => $item_id]);
        return $this->page('sysitem/editWeight.html', $pagedata);
    }
    ...
    public function saveWeight()
    {
        ...
        $data = input::get();
        if (!is_numeric($data['order_weight'])) {
            $this->end(false,app::get('sysitem')->_('权重必须为数字'));
        }
        if ($data['order_weight'] < 0) {
            $this->end(false,app::get('sysitem')->_('权重必须为不小于0的整数'));
        }
        if (!app::get('sysitem')->model('item')->save($data)) {
            $this->end(false,app::get('sysitem')->_('保存失败'));
        }
        $this->end(true,app::get('sysitem')->_('保存成功'));
    }
}
