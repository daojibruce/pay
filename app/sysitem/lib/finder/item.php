...
class sysitem_finder_item{
    ...
    public function column_op(&$col ,$list)
    {
        foreach($list as $k=>$row)
        {
            ...
            $url2 = '?app=sysitem&ctl=admin_item&act=editWeight&finder_id='.$_GET['_finder']['finder_id'].'&p[0]='.$row['item_id'];
            $target2 = 'dialog::  {title:\''.app::get('sysproofing')->_('权重编辑').'\', width:400, height:200}';
            $title2 = app::get('sysproofing')->_('权重编辑');

            $col[$k] = '&nbsp;&nbsp;<a href="' . $url2 . '" target="' . $target2 . '">' . $title2 . '</a>';
        }   
    }
