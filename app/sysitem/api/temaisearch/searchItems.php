 ...
 * item.search
 */
class sysitem_api_temaisearch_searchItems{
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'user_id' => ['type'=>'int','valid'=>'integer','description'=>'所属会员id','example'=>'','default'=>''],
    ...
    {
        $filterCols = ['item_id','user_id','cat_id','search_keywords','approve_status','brand_id','prop_index','is_selfshop','dlytmpl_id'];
        foreach( $filterCols as $col )
        {
            if( $params[$col] )
            {
                ...
                if( in_array($col,['item_id','brand_id','prop_index']) )
                {
    ...
    {
//        $objMdlItem = app::get('systemai')->model('item');
