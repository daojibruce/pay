...
class syslogistics_data_temaitmpl {
     ...
     * @return bool
     */
    public function addDlyTmpl($data,$userId)
    {
        $this->__check($data, $userId);
        $saveData = $this->__preData($data,$userId);
     ...
     * @return bool
     */
    public function updateDlyTmpl($data,$userId)
    {
        $this->__check($data, $userId);
        $saveData = $this->__preData($data,$userId);
     ...
     * @param string $userId 平台展销会员id
     *
     * @return bool|int  存在返回template_id | false 不存在
     */
    public function isExistsName($tmplname,$userId)
    {
        $data = $this->objMdldlytmpl->getRow('template_id',array('name'=>$tmplname,'user_id'=>$userId));
        return $data ? $data['template_id'] : false;
    }

    private function __preData($data,$userId)
    {
        if( $data['template_id'] )
        {
    ...
    private function __check($data, $userId)
    {
        ...
        //修改的该模板ID是否存在
        $template_id = $this->isExistsName($data['name'], $userId);
