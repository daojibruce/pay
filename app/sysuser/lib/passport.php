...
class sysuser_passport
{
    ...
    public function verifyLogin($loginName, $password )
    {
        ...
        $account = app::get('sysuser')->model('account')->getRow('user_id,login_password,disabled',$filter);
        ...
        if($account['disabled']) {
            throw new \LogicException(app::get('sysuser')->_('该账号已被禁用'));
        }

        //验证成功，增加登录时间
        app::get('sysuser')->model('account')->update(['lasttime' => time()], ['user_id' => $account['user_id']]);
    ...
    public function signupUser($data)
    {
        ...
        //获取唯一会员编号
        $user_no = kernel::single('base_unique')->get_unique_no('user_no');
    ...
    public function memInfo($userId)
    {
        if( !$this->memInfo[$userId] )
        {
            ...
            $account = app::get('sysuser')->model('account')->getRow('login_account,email,mobile,login_type,user_no,disabled,lasttime',array('user_id'=>$userId));
            ...
            $memInfo = ['userId' => $userId,
                ...
                'lasttime' => date('Y-m-d H:i:s', $account['lasttime']),
                'sex' => $sysUserInfo['sex'],
                ...
                'user_type' => $sysUserInfo['user_type'],
                'verify_status' => $sysUserInfo['verify_status'],
                'is_master' => $sysUserInfo['is_master'],
                'master_user_id' => $sysUserInfo['master_user_id'],
