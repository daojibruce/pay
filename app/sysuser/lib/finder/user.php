...
class sysuser_finder_user {
    ...
    {
        $ctl = $_GET['ctl'];
        if(in_array($ctl,array('admin_buyerSelectNotice','admin_companyNotice'))  ){
            foreach($list as $k=>$row) {
                $url = '?app=sysuser&ctl=admin_notice&act=addNotice&finder_id=' . $_GET['_finder']['finder_id'] . '&user_id=' . $row['user_id'];
                $target = 'dialog::  {title:\'' . app::get('sysshop')->_('') . '\', width:500, height:400}';
                $title = app::get('sysshop')->_('添加会员通知');
            ...
            foreach($list as $k=>$row)
            {
                ...
                if($row['user_type']=='1' ) {
                    ...
                    if($row['verify_status']!='1'){
                        $url = '?app=sysuser&ctl=admin_user&act=editCompanyInfo&finder_id=' . $_GET['_finder']['finder_id'] . '&p[0]=' . $row['user_id'].'&v='.$row['verify_status'];
                        $target = 'dialog::  {title:\'' . app::get('sysuser')->_('企业会员信息编辑') . '\', width:600, height:400}';
                        $title = app::get('sysuser')->_('编辑');
                        $button = '<a href="' . $url . '" target="' . $target . '">' . $title . '</a>';
                    }else{
                        $url = '?app=sysuser&ctl=admin_user&act=editCompanyInfo&finder_id=' . $_GET['_finder']['finder_id'] . '&p[0]=' . $row['user_id'].'&v='.$row['verify_status'];
                        $target = 'dialog::  {title:\'' . app::get('sysuser')->_('企业会员信息查看') . '\', width:600, height:400}';
                        $title = app::get('sysuser')->_('编辑');
                        $button = '<a href="' . $url . '" target="' . $target . '">' . $title . '</a>';
                    }
                ...
                if($row['verify_status']=='0'){
                    $url = '?app=sysuser&ctl=admin_user&act=checkInfo&finder_id=' . $_GET['_finder']['finder_id'] . '&p[0]=' . $row['user_id'].'&v='.$row['verify_status'];
                    $target = 'dialog::  {title:\'' . app::get('sysuser')->_('企业会员信息审核') . '\', width:600, height:400}';
                    $title = app::get('sysuser')->_('审核');
                    $button = '<a href="' . $url . '" target="' . $target . '">' . $title . '</a>';

                    /*
                    $url = '?app=sysuser&ctl=admin_user&act=checkCompanyInfo&finder_id='.$_GET['_finder']['finder_id'].'&user_id='.$row['user_id'];
                    $target = 'dialog::  {title:\''.app::get('sysuser')->_('企业会员审核').'\', width:500, height:100}';
                    $title = app::get('sysuser')->_('通过');
                    $button .= ' | <a href="' . $url . '" target="' . $target . '">' . $title . '</a>';

                    $url = '?app=sysuser&ctl=admin_user&act=refuseCompanyInfo&user_id='.$row['user_id'];
                    $target = 'dialog::  {title:\''.app::get('sysuser')->_('请填写驳回原因').'\', width:500, height:200}';
                    $title = app::get('sysuser')->_('不通过');
                    $button .= ' | <a href="' . $url . '" target="' . $target . '">' . $title . '</a>';
                    */
