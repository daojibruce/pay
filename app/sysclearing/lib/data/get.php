...
class sysclearing_data_get {
     ...
     * 根据条件获取平台展销列表
     *
     * @param array $userIds    角色类型,见userAuth.user_role config
     * @param array $userIds    会员id
     *
     * @return array
     * */
    public function getTemaiList($type = SYS_USER_ROLE_SPECICAL, $userIds = array())
    {
        ...
        if ($userIds)
        {
            ...
            $where .= " AND user_id in({$userIds_str})";
        }

        $role_config = config::get('userAuth.user_role');
        $role_config = array_keys($role_config);
        $roles = kernel::single('ectools_code')->init($role_config)->getIsxx(0, $type);

        $objMdlAccount = app::get('sysuser')->database();
        $sql = "SELECT user_id FROM sysuser_account WHERE user_role & {$roles} {$where}";
        $result = $objMdlAccount->executeQuery($sql)->fetchAll();

        if($result) $filter['user_id'] = $result;
        $objMdlUser = app::get ('sysuser')->model('user');
        $userList = $objMdlUser->getList('user_id,name', $filter);

        $temaiTempIds = array_column ($userList, 'user_id');
        $temaiTempName = array_column ($userList, 'name');
        $temaiList = array_combine ($temaiTempIds, $temaiTempName);
