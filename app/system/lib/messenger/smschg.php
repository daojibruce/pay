...
class system_messenger_smschg{
    ...
    /**
     * 服务器时间接口
     * http://sandbox.stl515.com/sms/(内网)
     * http://webapi.stl515.com(线上)
     */
    private $timeUrl = 'http://webapi.stl515.com';

    /**
     * 免登录地址
     * http://newsms.ex-sandbox.com(内网)
     * http://sms.stl515.com(线上)
     */
    private $passLoginUrl = 'http://sms.stl515.com/?';
     ...
     * http://api.sms.cn(线上)
     */
    private $accountUrl = 'http://api.sms.cn';
     ...
     * http://api.sms.cn(线上)
     */
    private $sendUrl = 'http://api.sms.cn/sms/';
     ...
     * http://webapi.stl515.com(线上)
     */
    private $codeUrl = 'http://webapi.stl515.com';

    public function __construct() {
        $this->httpClient = kernel::single('base_httpclient');
        $this->httpClient->set_timeout(6);
        $this->smsUid = 'stl515';
        $this->smsMd5Pwd = 'e712950bd4844074942db936ea792b5a';
        //启用测试数据
        //$this->testInit();
    }
     ...
     * @param void
     * @return void
     * $contents[0 => ['phones' => '13918734302' , 'content' => '您的验证码：127880请在页面中输入完成验证，短信有效期为60分钟【stl】']]
     * $config['sendType' => 'notice' , 'use_reply' => false]
     */
    public function send($contents,$config) {
        ...
        foreach($contents as $index => $content){
            $newContents[$index]['mobile'] = $content['phones'];
            $newContents[$index]['msg'] = $content['content'];
        }
        $newContents = json_encode($newContents);

        $data = ['ac' => 'sendsingle' , 'uid' => $this->smsUid , 'pwd' => $this->smsMd5Pwd , 'content' => $newContents];
        // ?ac=send&uid=用户账号&pwd=MD532位密码&mobile=号码&content=内容
    ...
    public function testInit() {
        ...
        //测试企业密码
        //$this->entac = md5('123jjzh'.'Stl515User');
    }
}
?>
