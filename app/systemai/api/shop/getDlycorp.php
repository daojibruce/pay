<?php
class systemai_api_shop_getDlycorp{

    public $apiDescription = "获取店铺签约物流";
    public function getParams()
    {
        $return['params'] = array(
            'user_id' =>['type'=>'int','valid'=>'int', 'description'=>'会员id','default'=>'','example'=>'1'],
            'corp_id' =>['type'=>'int','valid'=>'', 'description'=>'物流公司编号id','default'=>'','example'=>'1'],
            'fields' => ['type'=>'field_list','valid'=>'', 'description'=>'获取指定字段','default'=>'corp_id,corp_code,corp_name,user_id','example'=>'corp_id,corp_code,corp_name'],
    ...
    {
        if(!$params['user_id'] && !$params['corp_id'])
        {
            return array();
        }
        ...
        //默认查询字段
        $row = "corp_id,corp_code,corp_name,user_id";
        if($params['fields'])
        {
            $row = $params['fields'];
        }
