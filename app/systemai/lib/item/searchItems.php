 ...
 * item.search
 */
class systemai_item_searchItems{
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'user_id' => ['type'=>'int','valid'=>'integer','description'=>'会员id','example'=>'','default'=>''],
    ...
    {
        $filterCols = ['item_id','user_id','shop_cat_id','cat_id','search_keywords','approve_status','brand_id','prop_index','is_selfshop','dlytmpl_id'];
        ...
        //获取指定商铺类目下的所有类目
        if($params['search_shop_cat_id'] >0 && $params['user_id']){
            $catParams = array();
            $catParams['user_id'] = $params['user_id'];
            $catParams['parent_id'] = $params['search_shop_cat_id'];
            $catParams['fields'] = 'cat_id';
            $catIds = app::get('systemai')->rpcCall('shop.cat.get', $catParams);
    ...
    public function getList($params)
    {
        ...
        $data = kernel::single('search_object')->instance('temaiitem')
            ->page($page, $limit)
        ...
        {
            $itemStore = kernel::single('systemai_item_info')->getItemStore($itemIds);
        }
        ...
        if( $itemIds && $params['fields']['extends']['promotion'] )
        {
            ...
            $giftData = app::get('systemai')->rpcCall('promotion.gift.item.get',$giftFilter);
