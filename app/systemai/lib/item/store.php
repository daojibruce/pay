...
class systemai_item_store {
    public function getItemListByStore($row,$filter,$start,$limit,$orderBy)
    {
        ...
        $store->select('I.item_id,I.modified_time,I.title,I.image_default_id,I.dlytmpl_id,I.price,I.nospec,S.store,ST.approve_status,ST.list_time')
           ...
           ->andWhere('I.user_id='.$store->createPositionalParameter($filter['user_id']))
    ...
    //更新指定sku bn的库存
    public function updateStoreByBn($skuBn, $userId, $store)
    {
        $ids = $this->__getIdsByBn($skuBn, $userId);
        $itemId = $ids['item_id'];
        $skuId = $ids['sku_id'];
        return $this->updateStore($itemId, $skuId, $store);
    }

    private function __getIdsByBn($skuBn, $userId)
    {
        if( !$skuBn || !$userId ) return false;
        ...
        $filter = ['item_id|in'=>$itemIds, 'user_id'=>$userId];
        $itemId = $ItemModel->getRow('item_id', $filter);
        if( empty($itemId) )
        {
            throw new LogicException( "该平台展销中bn {$skuBn} 不存在" );
        }
