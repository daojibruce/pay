...
class systemai_data_item {
     ...
     * @return bool
     */
    public function batchDownTemai($params,$status,&$msg)
    {
        if($params['item_id'][0] == '_ALL_')  unset($params);
        $ojbMdlItem = app::get('systemai')->model('temai');
    ...
    {
        //判断商品规格号是否重复
        if (count($postData['sku']) != count(array_unique(array_column($postData['sku'],'spec_code')))) {
            $msg = app::get('sysitem')->_('存在重复的商品规格号');
            throw new \LogicException($msg);
        }
        // 修改情况下的各种判断
        if($postData['item_id'])
        {
            ...
            //判断商品规格号是否重复
            $skuInfo = app::get('sysitem')->model('sku')->getList('sku_id,spec_code',['item_id' => $postData['item_id']]);
            if ($skuInfo) {
                foreach ($skuInfo as $skuI) {
                    foreach ($postData['sku'] as $sku) {
                        if ($sku['spec_code'] == $skuI['spec_code'] && $sku['sku_id'] != $skuI['sku_id']) {
                            $msg = app::get('sysitem')->_('存在重复的商品规格号');
                            throw new \LogicException($msg);
                        }
                    }
            ...
            $itemInfo = $objMdlItem->getList('item_id,cat_id,nospec', [ 'item_id'=>$postData['item_id'], 'user_id'=>$postData['user_id'] ]);
            if(!$itemInfo)
            {
        ...
        // 判断运费模板
        $dlytmplInfo = app::get('sysitem')->rpcCall('logistics.dlytmpl.get', ['template_id'=>$postData['dlytmpl_id'], 'status'=>'on', 'user_id'=>$postData['user_id'], 'fields'=>'name']);
        if(!$dlytmplInfo)
        {
    ...
    private function __formatData( $postData )
    {
        ...
        if($postData['nospec'])
        {
            ...
            $postData['sku'][0]['user_id']      = $postData['user_id'];
            ...
            $postData['sku'][0]['length']     = $postData['length'];
            $postData['sku'][0]['width']     = $postData['width'];
            $postData['sku'][0]['height']     = $postData['height'];
            $postData['sku'][0]['spec_code']     = $postData['spec_code'];
            unset($postData['spec_code']);
        }
        ...
        foreach( $postData['sku'] as $prok => $pro )
        {
            ...
            $postData['sku'][$prok]['user_id']      = intval($postData['user_id']);
            ...
            $postData['sku'][$prok]['weight']     = trim($postData['sku'][$prok]['weight']);
            $postData['sku'][$prok]['length']     = trim($postData['sku'][$prok]['length']);
            $postData['sku'][$prok]['width']     = trim($postData['sku'][$prok]['width']);
            $postData['sku'][$prok]['height']     = trim($postData['sku'][$prok]['height']);
            /*$specKey = '';
            if(!$postData['nospec'])
            {
    ...
    private function __formatItemData( $postData )
    {
        ...
        if( $this->__checkProductBn($data['bn'], $postData['item_id'], $postData['user_id'], 'item') )
        {
        ...
        if($postData['item_id'])
        {
         ...
         // 判断店铺是不是自营店铺
        $postData['is_selfshop'] = 0;
        $data['user_id'] = $postData['user_id'];
        $data['protection_info'] = $postData['protection_info'];
    ...
    private function __formatItemNaturePorpsData( $postData, $item )
    {
        ...
        foreach($postData['nature_props'] as $k=>$v)
        {
            if(is_array($v)) {
                if ($v['is_new'] == '1') {
                    $info = app::get('syscategory')->model('prop_values')->getRow('prop_value_id',['prop_value' => $v['value']]);
                    if (!$info) {
                        $data = [
                            'prop_id' => $k,
                            'prop_value' => $v['value'],
                        ...
                        $prop_value_id = app::get('syscategory')->model('prop_values')->insert($data);
                    } else {
                        $prop_value_id = $info['prop_value_id'];
                    }
                    $data = array(
                        'item_id' => $item['item_id'],
                        'prop_id' => $k,
                        'prop_value_id' => $prop_value_id,
                        'pv_number' => $prop_value_id,
                        'pv_type' => 'text',
                        'pv_str' => '',
                        'modified_time' => time(),
                    );
                    ...
                    if ($objMdlItemNatureProps->getRow('*',['item_id' => $item['item_id'], 'prop_id' => $k])) {
                        $data = [
                            'prop_value_id' => $v['prop_value_id'],
                            'prop_id' => $k,
                            'prop_value' => $v['value'],
                            'order_sort' => '0'
                        ];
                        app::get('syscategory')->model('prop_values')->save($data);
                    }
                }
                $data = [
                    'prop_id' => $k,
    ...
    private function __checkProductBn($bn, $primaryid=0, $userId=0, $type=""){
        if(empty($bn)){
            return false;
        }
        ...
        if($type == "item")
        {
            $data = $ojbMdlItem->getRow("item_id",['bn'=>$bn,'user_id'=>$userId,'item_id|noequal'=>$primaryid]);
            if($data) return true;
        }
        if($type == "sku")
        {
            $data = $ojbMdlSku->getRow("item_id",['bn'=>$bn,'user_id'=>$userId,'sku_id|noequal'=>$primaryid]);
            if($data) return true;
        }
        return false;
    }
