 ...
 */
class systemai_search_store {

    public function getItemListByStore($params, $start, $limit)
    {
        $store = app::get ('systemai')->database ()->createQueryBuilder ();
        ...
        ->from ('systemai_item_store', 'S')
        ->leftJoin ('S', 'systemai_item', 'I', 'S.item_id=I.item_id')
        ->leftJoin ('S', 'systemai_item_status', 'ST', 'S.item_id=ST.item_id')
        ->where ('S.store<=' . $store->createPositionalParameter ($params ['store']))
        ->andWhere ('I.user_id=' . $store->createPositionalParameter ($params ['user_id']))
    ...
    public function getItemCountByStore($params)
    {
        ...
        $store = app::get ('systemai')->database ()->createQueryBuilder ();
        $db = $store->getConnection ();
        $store->select ('count(I.item_id) as itemNum')->from ('systemai_item_store', 'S')->leftJoin ('S', 'systemai_item', 'I', 'S.item_id=I.item_id')->leftJoin ('S', 'systemai_item_status', 'ST', 'S.item_id=ST.item_id')->where ('S.store<=' . $store->createPositionalParameter ($params ['store']))->andWhere ('I.user_id=' . $store->createPositionalParameter ($params ['user_id']));
