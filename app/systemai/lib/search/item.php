...
class systemai_search_item  {
    ...
    private function __preMysqlSearchFilter($filter)
    {
        ...
        {
            $shopIds = app::get('systemai')->rpcCall('shop.get.search',['shop_name'=>$filter['shop_name'],'fields'=>'user_id']);
            if( $shopIds )
            {
                $filter['user_id'] = array_column($shopIds,'user_id');
            }
            else
            {
                $filter['user_id'] = '-1';
        ...
        {
            $catIds = app::get('systemai')->rpcCall('category.cat.get.info',['cat_name'=>$filter['cat_name'],'level'=>3,'fields'=>'cat_id']);
            if( $catIds )
            {
        ...
        {
            $brandIds = app::get('systemai')->rpcCall('category.brand.get.list',['brand_name'=>$filter['brand_name'],'fields'=>'brand_id']);
            if( $brandIds )
            {
