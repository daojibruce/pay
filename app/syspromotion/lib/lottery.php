...
class syspromotion_lottery
{
    ...
    private function checkLotteryParams($params)
    {
        ...
        if( count($params['lotteryrules']) > 10 )
        {
            throw new \LogicException('最多添加10个奖项规则');
        }
        ...
        if($params['status'] == 'active'){
            $lotteryList = $this->objMdlLottery->getList('lottery_id,lottery_name,status');
            $statusCount = array_count_values(array_column($lotteryList, 'status', 'lottery_id'));
            if($statusCount['active'] >=5){
