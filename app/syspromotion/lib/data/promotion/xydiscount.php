...
class syspromotion_data_promotion_xydiscount extends syspromotion_abstract_promotions {
    ...
    public function save()
    {
        ...
        if( $data['xydiscount_id'] )
        {
            ...
            $this->__saveFulldiscountItem($data['xydiscount_id'], $promotionId);
        }
     ...
     */
    private function __saveFulldiscountItem($xydiscountId, $promotionId)
    {
        $objMdlXydiscountItem = app::get('syspromotion')->model('xydiscount_item');
        // 先删除满减关联的商品
        $objMdlXydiscountItem->delete(array('xydiscount_id'=>$fulldiscountId));
