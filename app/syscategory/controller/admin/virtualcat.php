...
class syscategory_ctl_admin_virtualcat extends desktop_controller {
    ...
    {
        $objMdlCat = app::get('syscategory')->model('virtualcat');
    ...
    public function add($nCatId = 0)
    {
        ...
        $virtualCatInfo = $objMdlCat->getRow('cat_path, level, virtual_cat_name, virtual_cat_logo, virtual_cat_id, virtual_parent_id', array('virtual_cat_id'=>$nCatId));
        ...
        {
            $pagedata['level'] = 3;
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, virtual_cat_name, virtual_cat_logo, virtual_cat_id, virtual_parent_id', array('virtual_cat_id'=>$virtualCatInfo['virtual_parent_id']));
    ...
    public function edit($nCatId, $is_leaf=0)
    {
        ...
        $virtualCatInfo = $objMdlVirtualcat->getRow('cat_path, level, virtual_cat_name, virtual_cat_logo, virtual_cat_id, virtual_parent_id, order_sort, filter,virtual_cat_template', array('virtual_cat_id'=>$nCatId));

        // 组织上级分类数据，1级分类显示无，2级分类显示1级分类,3级分类则显示对应的2级和1级分类
        $catList[0] = array('virtual_cat_id'=>0, 'virtual_cat_name'=>app::get('syscategory')->_('----无----'), 'step'=>1);
        if($virtualCatInfo['level']=='2')
        {
            $level1CatInfo = $objMdlVirtualcat->getRow('cat_path, level, virtual_cat_name, virtual_cat_id, virtual_parent_id, order_sort,virtual_cat_template', array('virtual_cat_id'=>$virtualCatInfo['virtual_parent_id']));
            $catList[1] = array('virtual_cat_id'=>$level1CatInfo['virtual_cat_id'], 'virtual_cat_name'=>$level1CatInfo['virtual_cat_name'], 'step'=>1);
        }
        if($virtualCatInfo['level']=='3')
        {
            $level2CatInfo = $objMdlVirtualcat->getRow('cat_path, level, virtual_cat_name, virtual_cat_id, virtual_parent_id,  order_sort', array('virtual_cat_id'=>$virtualCatInfo['virtual_parent_id']));
            $level1CatInfo = $objMdlVirtualcat->getRow('cat_path, level, virtual_cat_name, virtual_cat_id, virtual_parent_id, order_sort', array('virtual_cat_id'=>$level2CatInfo['virtual_parent_id']));
            $catList[1] = array('virtual_cat_id'=>$level1CatInfo['virtual_cat_id'], 'virtual_cat_name'=>$level1CatInfo['virtual_cat_name'], 'step'=>1);
    ...
    public function save()
    {
        ...
        $virtualCatInfo = $objMdlVirtualcat->getRow('virtual_cat_id', array('virtual_cat_name'=>$postData['virtual_cat_name'],'virtual_parent_id'=>$postData['virtual_parent_id']));
        ...
        else
        {
            try{
                ...
                $result = kernel::single('syscategory_data_virtualcat')->toSave($postData);
    ...
    public function toRemove()
    {
        ...
        try
        {
            $flag = kernel::single('syscategory_data_virtualcat')->toRemove(intval($nCatId));
        ...
        $objMdlVirtualcat = app::get('syscategory')->model('virtualcat');
        $virtualCatInfo = $objMdlVirtualcat->getRow('virtual_cat_name', array('virtual_cat_id'=>intval($nCatId)));
    ...
    public function updateSort()
    {
        ...
        $this->begin('?app=syscategory&ctl=admin_virtualcat&act=index');
        $objMdlVirtualcat = kernel::single('syscategory_data_virtualcat');
        $objMdlVirtualcat->updateSort($postData);
    ...
    /**
     * 判断是否是三级节点，即叶子节点
     * @param  int  $virtual_cat_id 分类id(三级分类)
     * @param  int $virtual_parent_id 返回三级分类父类id
     * @return boolean
     */
    public function isLeafCat($virtual_cat_id, &$virtual_parent_id)
    {
        if(!$virtual_cat_id) return false;

        $objMdlVirtualcat = app::get('syscategory')->model('virtualcat');
        $virtualCatInfo = $objMdlVirtualcat->getRow('is_leaf, virtual_parent_id', array('virtual_cat_id'=>$virtual_cat_id));
        $virtual_parent_id = $virtualCatInfo['virtual_parent_id'];
        return $virtualCatInfo['is_leaf'] ? true : false;
    }
    
    function checkCatName()
    {
        $objMdlVirtualcat = app::get('syscategory')->model('virtualcat');
        $catId = $objMdlVirtualcat->getRow('virtual_cat_name, virtual_cat_id', array( 'name'=>$_POST['name'] ) );
        if( $catId && $_POST['virtual_cat_id'] != $catId )
        {
            echo 'false';
        }
        else
