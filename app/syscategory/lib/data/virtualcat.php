...
class syscategory_data_virtualcat {
    ...
    {
        $aCats = $this->objMdlCat->getRow('virtual_cat_id', array('virtual_parent_id'=>intval($catId)));
        if($aCats)
        {
            $msg = '删除失败：本分类下面还有子分类';
        ...
        $parentRow = $this->objMdlCat->getRow('virtual_parent_id', array('virtual_cat_id'=>intval($catId)));

        $db = app::get('syscategory')->database();
        $db->beginTransaction();
        try
        {
            $result = $this->objMdlCat->database()->delete('syscategory_virtualcat', ['virtual_cat_id' => $catId], [\PDO::PARAM_INT]);
            if(!$result) throw new \LogicException("删除类目失败");

            if($parentRow['virtual_parent_id'])
            {
                $result = $this->objMdlCat->database()->executeUpdate('UPDATE syscategory_virtualcat SET child_count = child_count-1 WHERE virtual_cat_id=?', [$parentRow['virtual_parent_id']], [\PDO::PARAM_INT]);
    ...
    public function updateSort($sortData)
    {
        ...
        {
            $this->objMdlCat->update( array('order_sort'=>($v==='' ? null : $v)), array('virtual_cat_id'=>$k) );
            event::fire('virtualcat.save');
        }
        return true;
    }
    ...
    {
        $tree = redis::scene('system')->get('virtualcat');
        if($tree){
    ...
    {
        set_time_limit(2000);
        $data = app::get('syscategory')->model('virtualcat')->getList('virtual_cat_id,virtual_parent_id,virtual_cat_name,url,cat_path,level,is_leaf,child_count,order_sort',array(),0,-1,'order_sort ASC, virtual_cat_id DESC');
    ...
    public function makeTree()
    {
        ...
        redis::scene('system')->set('virtualcat',serialize($tree));
        return $tree;
    }
