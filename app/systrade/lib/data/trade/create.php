   ...
   /**
    ...
    public function generate($tradeParams, $aCart=array() )
    {
        ...
        try
        {
            ...
            //计算订单价格，运费
            if ($tradeParams['offer_id']) {
                $priceTotalData[$tradeParams['shop_id']] = [
                    'payment' => $tradeParams['total_fee'],
                    'total_fee' => $tradeParams['sample_fee'],
                    'discount_fee' => 0,
                    'obtain_point_fee' => 0,
                    'post_fee' => $tradeParams['post_fee'],
                ];
                $tradeData[$tradeParams['shop_id']]['shipping_type'] = 'express';
                $tradeData[$tradeParams['shop_id']]['dlytmpl_ids'] = 0;
                $tradeData[$tradeParams['shop_id']]['contract_status'] = 0;
                unset($tradeParams['post_fee']);
                unset($tradeParams['sample_fee']);
                unset($tradeParams['total_fee']);
                unset($tradeParams['shop_id']);
            } else {
            ...
            foreach( $tradeData as $shopId=>$row )
            {
                //平台展销特殊处理
                if($shopId == 'temai') {
                    foreach($row AS $user_id=>$dd) {
                        $tradeData[$shopId][$user_id] = array_merge($priceTotalData[$shopId][$user_id], $dd);
                    }
                    continue;
                }
            ...
            //处理积分抵扣后的订单数据 | 平台展销暂不支持积分抵扣
            if( $tradeParams['use_points'] && !isset($tradeData['temai']))
            {
            ...
            foreach( $tradeData as $shopId=>$row )
            {
                ...
                if($shopId == 'temai') {
                    foreach($row AS $user_id=>$dd) {
                        $result = $this->objMdlTrade->save($dd);
                        if(!$result) throw new \LogicException(app::get('systrade')->_('订单生成失败'));
                    }
                }
                //普通订单处理
                else {
                    $result = $this->objMdlTrade->save($row);
    ...
    private function __commonTradeData($tradeParams)
    {
        ...
        return [
            ...
            'need_econtract'	=> $tradeParams['need_econtract'],//是否需要电子合同
            'contract_status'   => $tradeParams['contract_status']//合同状态
        ];
    }
     ...
     * @param array $shopId
     */
    private function __orderItemData($tradeParams, $cartItem, $shopId, $isSpec = false)
    {
        ...
        $orderData = [
            ...
            'created_time'     => $this->getTime(),
            'modified_time'    => $this->getTime(),
            'status'           => $this->__tradeStatus($tradeParams['payment_type'] , $tradeParams['need_econtract']),
        ...
        $isSpec && $orderData['temai_user_id'] = $shopId;
     ...
     * @return string
     */
    private function __tradeStatus( $paymentType , $needEcontract = false)
    {
        //如果订单的支付方式为线下支付，订单状态默认为等待发货，否则为等待支付
        if($needEcontract){
            return "WAIT_SENDCONTRACT";
        }
        return ($paymentType == "offline" || $paymentType == "caroffline") ?  "WAIT_SELLER_SEND_GOODS" : "WAIT_BUYER_PAY";
    }
    ...
    public function getTradeTotal($tradeParams, $aCart)
    {
        ...
        {
            if($shopId == 'temai') {
                $priceTemaiTotalData = $this->getTemaiTradeTotal($tradeParams, $shopCartData);
                $priceTotalData = array_merge($priceTotalData, $priceTemaiTotalData);
                continue;
            }
     ...
     * @param array $aCart 需要创建订单获取的购物车数据
     */
    public function getTemaiTradeTotal($tradeParams, $aCart)
    {
        foreach ($aCart as $userId => $shopCartData )
        {
            //计算订单总金额
            $totalParams = array(
                'discount_fee' => $shopCartData['cartCount']['total_discount'],
                ...
                'total_weight' => $shopCartData['cartCount']['total_weight'],
                'shop_id' => 0,
                //'shipping_type' => $tradeParams['shipping'][$userId]['shipping_type'],
            ...
            $priceTotalData[$userId]['payment'] = $totalInfo['payment'];
            $priceTotalData[$userId]['total_fee'] = $totalInfo['total_fee'];
            $priceTotalData[$userId]['discount_fee'] = $totalInfo['discount_fee'];
            $priceTotalData[$userId]['obtain_point_fee'] = $totalInfo['obtain_point_fee'];
            $priceTotalData[$userId]['post_fee'] = $totalInfo['post_fee'];
        }

        return ['temai' => $priceTotalData];
    }
    ...
    private function _chgdata( $tradeParams, $aCart=array() )
    {
        ...
        foreach ($aCart['resultCartData'] as $shopId => $shopCartData ) {
            if ($shopId == 'temai') {
                $this->__chgTemaiData($tradeParams, $shopCartData, $orderData);
                continue;
            }
            ...
            $shopTradeData = [
                'shop_id' => $shopId,
                'tid' => $tid,
                'status' => $this->__tradeStatus($commonTradeData['pay_type'], $tradeParams['need_econtract']),
                ...
                'shipping_type' => $tradeParams['shipping'][$shopId]['shipping_type'],
                'total_weight' => $shopCartData['cartCount']['total_weight'],
                'contract_id' => $tradeParams['contract_id'][$shopId],//合同ID
            ];
            ...
            // 子订单
            if ($tradeParams['offer_id']) {
                $orderData[$shopId]['order_type'] = 'proofing';
                $orderData[$shopId]['order'][0] = [
                    'oid' => $this->genId($tradeParams['user_id'], false),
                    'tid' => $tid,
                    'shop_id' => $shopCartData['shop_id'],
                    'user_id' => $tradeParams['user_id'],
                    'item_id' => $shopCartData['object'][0]['item_id'],
                    'sku_id' => $shopCartData['object'][0]['sku_id'],
                    'cat_id' => $shopCartData['object'][0]['cat_id'],
                    'bn' => $shopCartData['object'][0]['bn'],
                    'price' => $shopCartData['object'][0]['price']['price'],
                    'num' => $shopCartData['object'][0]['quantity'],
                    'payment' => $shopCartData['object'][0]['price']['total_price'],
                    'total_fee' => $shopCartData['object'][0]['price']['total_price'],
                    'total_weight' => 1,
                    'part_mjz_discount' => 0,
                    'sub_stock' => $shopCartData['object'][0]['quantity'],
                    'sendnum' => 0,
                    'created_time' => time(),
                    'modified_time' => time(),
                    'status' => 'WAIT_BUYER_PAY',
                    'title' => $shopCartData['object'][0]['title'],
                    'order_from' => 'pc',
                    'dlytmpl_id' => 0,
                ];
            } else {
    ...
    private function __chgTemaiData($tradeParams, $aCart=array(), &$orderDatas) {
        $commonTradeData = $this->__commonTradeData($tradeParams);
        ...
        foreach ($aCart as $userId => $shopCartData ) {
            ...
            $shopTradeData = [
                'shop_id' => 0,
                'temai_user_id' => $userId,
                'tid' => $tid,
                'status' => $this->__tradeStatus($commonTradeData['pay_type'], $tradeParams['need_econtract']),
                'trade_memo' => strip_tags($tradeParams['trade_memo'][$userId]),
                'dlytmpl_ids' => implode(',', array_keys($shopCartData['cartByDlytmpl'])),
                'ziti_addr' => $tradeParams['ziti'][$userId]['ziti_addr'],
                'itemnum' => $shopCartData['cartCount']['itemnum'],
                'shipping_type' => $tradeParams['shipping'][$userId]['shipping_type'],
                'total_weight' => $shopCartData['cartCount']['total_weight'],
                'contract_id' => $tradeParams['contract_id'][$userId],//合同ID
            ];

            //主订单基本数据 不包含订单金额数据
            $orderData[$userId] = array_merge($commonTradeData, $shopTradeData);

            /*
            $couponCode = kernel::single('systrade_cart_coupon_redis')->get($tradeParams['user_id'], $userId);
            //当前订单使用的店铺优惠券 用于记录使用店铺优惠券日志
            if ($shopCartData['cartCount']['total_coupon_discount'] > 0 && $couponCode) {
                $this->cartUseCoupon[$userId] = $couponCode;
            }
            //购物车优惠信息
            $this->__setPromotionParams($userId, $shopCartData);
            */

            // 子订单
            if ($tradeParams['offer_id']) {
                $orderData[$userId]['order_type'] = 'proofing';
                $orderData[$userId]['order'][0] = [
                    'oid' => $this->genId($tradeParams['user_id'], false),
                    'tid' => $tid,
                    'shop_id' => 0,
                    'user_id' => $tradeParams['user_id'],
                ...
                foreach ($shopCartData['object'] as $k => $cartItem) {
                    if ($cartItem['obj_type'] == 'item') {
                        $shopOrderData = $this->__orderItemData($tradeParams, $cartItem, $userId, 1);
                        $this->__minusStore($shopOrderData);
                        $orderData[$userId]['order'][] = $shopOrderData;
                    }
                    $this->cartIds[] = $cartItem['cart_id'];
                }
            }
            $orderDatas['temai'] = $orderData;
        }
    }
    ...
    private function __minusStore($orderData)
    {
        ...
        //$isMinus2 = app::get('systrade')->rpcCall('item.store.minus',$params);
        //下单减平台展销库存
        //todo..
