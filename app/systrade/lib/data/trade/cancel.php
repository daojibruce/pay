...
class systrade_data_trade_cancel
{
    ...
    public function create($tid, $cancelReason, $refundBn,$returnFreight=null)
    {
        ...
        $db->beginTransaction();
        try {
            $noPayStatus = array('WAIT_BUYER_PAY','WAIT_SENDCONTRACT','WAIT_CONFIRM','REJECT_CONTRACT');
            //未付款 可直接取消 货到付款并且不是消费者申请取消订单则可以直接取消
            if( in_array($tradeDataInfo['status'] , $noPayStatus) || ($tradeDataInfo['pay_type'] == 'offline' && $this->getCancelFromType() != 'buyer' ) )
            {
