...
class systrade_data_trade_confirm {
     ...
     * @return boolean - 成功与否
     */
    public function generate($tid, $userId=null, $shopId=null , $temai_user_id=null)
    {
        $filter['tid'] = $tid;
        if( $userId ) $filter['user_id'] = intval($userId);
        if( $shopId ) $filter['shop_id'] = intval($shopId);
        if( $temai_user_id ) $filter['temai_user_id'] = intval($temai_user_id);

        $tradeInfo = $this->objTrade->getTradeInfo('status,user_id,shop_id,temai_user_id,obtain_point_fee,consume_point_fee,payment,post_fee,points_fee,cancel_status',$filter);
