...
class systrade_data_cart {
     ...
     *
     * @param array $params 加入购物车参数
     * @param boolean $isSpec 是否平台展销
     *
     * @return bool
     */
    public function addCart($params, $isSpec = 0)
    {
        ...
        // 进行各自的特殊校验
        $is_temai = $this->objects[$params['obj_type']]->checkIsTemai('', $params['sku_id']);

        $this->objects[$params['obj_type']]->checkObject($mergeParams, $cartBasicData, $is_temai);
        ...
        if($this->userId == $is_temai) {
            throw new Exception("不能购买自己的商品!", 1);
        }

        //增加平台展销id
        if($is_temai) $data['temai_user_id'] = $is_temai;
    ...
    public function fastBuyStore($params)
    {
        ...
        $tmp = cache::store('session')->get($key);
        if (!$tmp) {
            $data[] = $params;
            cache::store('session')->put($key, $data, 3600);
        } else {
            if ($params['ftime'] == $tmp[0]['ftime']) {
                $tmp[] = $params;
                cache::store('session')->put($key, $tmp, 3600);
            } else {
                $data[] = $params;
                cache::store('session')->put($key, $data, 3600);
            }
    ...
    public function updateCart($params)
    {
        ...
        //检查商品是否能加入购物车
        $is_temai = $basicCartData[0]['temai_user_id'];
        $this->objects[$params['obj_type']]->checkObject($params, $basicCartData, $is_temai);
     ...
     * @return array
     */
    public function getCartInfo2($filter=array(), $needInvalid=true, $platform='pc')
    {
        $aCart = array();
        if(!$needInvalid) $filter['is_checked'] = '1';
        $cartData = $this->getBasicCart($filter);
        ...
        $result = $this->__preCartInfo($cartData, $needInvalid, $platform, 1);
        $aCart['resultCartData'] = $result['resultCartData'];
        $aCart['totalCart'] = $result['totalCart'];
        if( empty($aCart['resultCartData']) ) $aCart = array();
     ...
     * @return array
     */
    private function __preCartInfo($cartData, $needInvalid, $platform, $isSpec = false)
    {
        $temaiCartData = $shopIds = $newCartData = [];

        foreach( $cartData as $row)
        {
            // 临时解决是否平台展销商品
            $temai = app::get('systrade')->rpcCall('temai.item.get', array(
                'item_id' => $row['item_id'],
            ));
            if(isset($temai['sku'][$row['sku_id']])) {
                $row['selected_promotion'] = '0'; // 平台展销取消其他优惠
                $row['is_temai'] = true;
                $row['temai'] = $temai;

                //特殊下单流程
                if($isSpec) {
                    $temaiCartData['temai'][$row['temai_user_id']][] = $row;
                }
                //普通购物车显示流程
                else {
                    $temaiCartData['temai'][] = $row;
                }
            } else {
                $shopIds[] = $row['shop_id'];
        ...
        // 处理平台展销数据
        foreach( $temaiCartData AS $temaiData2 )
        {
            if($isSpec) {
                $baseResult1 = [];
                foreach($temaiData2 AS $temai_user_id => $temaiData) {
                    $temaiObjectData = $this->__preTemaiCartInfo($temaiData, $needInvalid, $platform);
                    if ($temaiObjectData) {
                        $resultTmp['shop_id'] = null;
                        $resultTmp['shop_name'] = '平台展销商品_' . $temai_user_id;
                        $resultTmp['shop_type'] = null;
                        // 统计购物车的总数量，总价格，促销价格等综合信息
                        $cartTotalInfo = $this->__cartTotal($temaiObjectData, 0, $needInvalid, $platform);

                        $resultTmp['cartCount'] = $cartTotalInfo['cartCount'];
                        $resultTmp['basicPromotionListInfo'] = $cartTotalInfo['basicPromotionListInfo'];
                        $resultTmp['usedCartPromotion'] = $cartTotalInfo['usedCartPromotion'];
                        $resultTmp['usedCartPromotionWeight'] = $cartTotalInfo['usedCartPromotionWeight'];
                        $resultTmp['cartByPromotion'] = $cartTotalInfo['cartByPromotion'];
                        $resultTmp['cartByDlytmpl'] = $cartTotalInfo['cartByDlytmpl'];
                        $resultTmp['object'] = $temaiObjectData;

                        $baseResult1[$temai_user_id] = $resultTmp;
                    }
                }
                $resultCartData['temai'] = $baseResult1;
            } else {
                $temaiObjectData = $this->__preTemaiCartInfo($temaiData2, $needInvalid, $platform);
                if( $temaiObjectData )
                {
                    $resultCartData['temai']['shop_id'] = null;
                    $resultCartData['temai']['shop_name'] = '平台展销商品';
                    $resultCartData['temai']['shop_type'] = null;
                    // 统计购物车的总数量，总价格，促销价格等综合信息
                    $cartTotalInfo = $this->__cartTotal($temaiObjectData, 0, $needInvalid, $platform);

                    $resultCartData['temai']['cartCount'] = $cartTotalInfo['cartCount'];
                    $resultCartData['temai']['basicPromotionListInfo'] = $cartTotalInfo['basicPromotionListInfo'];
                    $resultCartData['temai']['usedCartPromotion'] = $cartTotalInfo['usedCartPromotion'];
                    $resultCartData['temai']['usedCartPromotionWeight'] = $cartTotalInfo['usedCartPromotionWeight'];
                    $resultCartData['temai']['cartByPromotion'] = $cartTotalInfo['cartByPromotion'];
                    $resultCartData['temai']['cartByDlytmpl'] = $cartTotalInfo['cartByDlytmpl'];
                    $resultCartData['temai']['object'] = $temaiObjectData;
                }
            }
        }
        if(!empty($shopIds)) {
    ...
    private function __cartTotal(&$shopObjectData, $shop_id, $needInvalid, $platform)
    {
        ...
        foreach ($shopObjectData as $basicCartInfo)
        {
            ...
            {
                if (!$basicCartInfo['cart_id']) $basicCartInfo['cart_id'] = 0;
        ...
        // 应用优惠券(结算页，购物车页不应用)
        if(($couponCode = $this->getCouponCode($shop_id)))
        {
     ...
     * @return array
     */
    private function __preTemaiCartInfo($shopCartData, $needInvalid, $platform)
    {
        ...
        if($itemIds && $skuIds)
        {
            ...
            $skuRows = 'sku_id,bn,item_id,spec_info,price,weight,status,step_price';
        ...
        foreach( $shopCartData as $key => $row )
        {
            $temaiinfo = $row['temai'];
            if ($temaiinfo && !empty($temaiinfo)) {
                foreach ($skusData as $key => $sku) {
                    if(isset($temaiinfo['sku'][$sku['sku_id']])) {
                        $skusData[$key] = array_merge($skusData[$key], array(
                            'price' => $temaiinfo['sku'][$sku['sku_id']]['price'],
                            'mkt_price' => $temaiinfo['sku'][$sku['sku_id']]['mkt_price'],
                            'realStore' => $temaiinfo['sku'][$sku['sku_id']]['store'],
                            'store' => $temaiinfo['sku'][$sku['sku_id']]['store'],
                            'saleStore' => $temaiinfo['sku'][$sku['sku_id']]['sale_num'],
                        ));
                    }
                }
            }
            $k = $row['cart_id'] ? $row['cart_id'] : $key;
    ...
    private function __preShopCartInfo($shopCartData, $needInvalid, $platform)
    {
        ...
        if($itemIds && $skuIds)
        {
            ...
            foreach ($skusData as $key => $sku) {
                if ($sku['step_price']) {
                    $skusData[$key]['step_price'] = unserialize($sku['step_price']);
                    //排序
                    $arrSort = [];
                    foreach ($skusData[$key]['step_price'] as $uniqid => $row){
                        foreach ($row as $k => $value){
                            $arrSort[$k][$uniqid] = $value;
                        }
                    }
                    array_multisort($arrSort['quantity'], constant('SORT_DESC'), $skusData[$key]['step_price']);
                }
            }
        ...
        foreach( $shopCartData as $key => $row )
        {
            ...
            //添加阶梯价格过滤
            if ($skusData[$row['sku_id']]['step_price']) {
                foreach ($skusData[$row['sku_id']]['step_price'] as $k => $v) {
                    if ($row['quantity'] >= $v['quantity']) {
                        $skusData[$row['sku_id']]['price'] = $v['price'];
                    }
                }
            }
            $shopObjectData[$k] = $this->objects[$row['obj_type']]->processCartObject($row,$itemsData,$skusData,$itemActivityList);
            //echo "<pre>";print_r($skusData);exit;
            $itemId = $row['item_id'];
