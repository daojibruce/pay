...
class systrade_cart_object_item implements systrade_interface_cart_object{
     ...
     * @param string $isSpec 是否特殊商品，0普通商品,1平台展销商品
     * @return boolean true or false
     */
    public function checkObject($params, $basicCartData, $isSpec = false)
    {
        // 如果是更新购物车则获取sku_id
        if($params['cart_id'])
        {
        ...
        //检查加入购物的商品是否有效
        if( !$this->__checkItemValid($itemData, $skuData, $isSpec) )
        {
     ...
     * @param boolean $isSpec   是否跳过特殊验证
     *
     * @return bool
     */
    private function __checkItemValid($itemsData, $skuData, $isSpec = false)
    {
    ...
    public function processCartObject($row,$itemsData,$skusData,$itemActivityList)
    {
        ...
        {
            $objectData['valid'] = $this->__checkItemValid($itemsData[$itemId], $skusData[$skuId], $row['is_temai']);//是否为有效数据
        }
    ...
    public function checkIsTemai($item_id = 0, $sku_id = 0) {
        ...
        if($sku_id) {
            ...
            if($skuInfo) {
                $itemInfo = $this->getTemaiItem($skuInfo['item_id'], 'item_id,user_id');
                if($itemInfo) return $itemInfo['user_id'];
