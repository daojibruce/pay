...
class topdev_ctl_apis extends topdev_controller {
    ...
    //接口分类
    private $_apiNameList = [
        'topapi' => ['APP聚合API列表', 'APP聚合API'],
        'omsapi' => ['OMS系统对接API列表', 'OMS系统对接API'],
        'apis' => ['系统API列表', '系统API'],
    ];
    ...
    public function group()
    {
        ...
        if( $apiType == 'topapi' || $apiType == 'omsapi')
        {
            $list = kernel::single('topdev_apis')->getTopApiGroupList(input::get('v','v1'));
            $this->runtimePath[] = ['title' => app::get('topdev')->_($this->_apiNameList[$apiType][0])];
            $this->runtimePath[] = ['title' => input::get('v','v1')];
            $this->activeMenu = $this->_apiNameList[$apiType][1];
        }
        else
        {
    ...
    public function testApi()
    {
        ...
        elseif( $apiType == 'omsapi' )
        {
            $url = kernel::base_url(1).kernel::url_prefix().'/omsapi';
            $pagedata['apiParams'] = $apiParams;
            $apiParams['method'] = $method;
            $apiParams['timestamp'] = time();
            $apiParams['sign_type'] = 'MD5';
