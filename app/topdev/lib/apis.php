...
class topdev_apis {
    ...
    public function getApiGroupList($type = 'apis')
    {
        $apis = $this->getApiList($type);

        $redis = redis::scene('topdev');
        $redisKey = $type . md5(json_encode($apis));
     ...
     * 获取API列表
     *
     * @param string $type API类型:apis系统API，topapi聚合API，OMS交互API
     */
    public function getApiList($type, $v='v1')
    {
        ...
        elseif($type == 'omsapi') {
            $apis = config::get('omsapi.routes.' . $v);
        }
        else
        {
