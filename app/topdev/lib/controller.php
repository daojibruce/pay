...
class topdev_controller extends base_routing_controller {
    ...
    public function getMenu()
    {
        ...
        $devMenu->group(array('group_name' => 'API管理'), function() use ($devMenu) {
            ...
            $devMenu->group(array('group_name' => 'OMS系统对接API','icon'=>'fa fa-th-oms'), function() use ($devMenu) {
                $list = kernel::single('topdev_apis')->getApiGroupList('omsapi');
                foreach( $list as $row )
                {
                    $devMenu->add($row['name'], array('topdev_ctl_apis@group',['group'=>$row['name'],'apitype'=>'omsapi']) ,$row['count']);
                }
            });
