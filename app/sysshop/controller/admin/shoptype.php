...
class sysshop_ctl_admin_shoptype extends desktop_controller {
    ...
    public function delete($shoptype_id)
    {
        $this->begin('?app=sysshop&ctl=admin_shoptype&act=index');
        $filter = array(
            'shoptype_id'=>$shoptype_id,
        );
        $shoptypeMdl = app::get('sysshop')->model('shop_type');
        $shoptype = $shoptypeMdl->getRow('*',$filter);
        if (app::get('sysshop')->model('shop')->getRow('shop_id',['shop_type' => $shoptype['shop_type']]))
        {
            $this->end(false,'该类型下存在店铺，无法删除！');
        } else {
            if ($shoptypeMdl->delete(['shoptype_id' => $shoptype_id])){
                $this->end(true);
            } else {
                $this->end(false,'删除失败!');
            }
