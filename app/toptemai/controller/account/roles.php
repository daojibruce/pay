...
class toptemai_ctl_account_roles extends toptemai_controller {
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('角色管理');
        $params['shop_id'] = $this->shopId;
        $data = app::get('toptemai')->rpcCall('account.shop.roles.list',$params);
        $pagedata['data'] = $data;
        return $this->page('toptemai/account/roles/list.html', $pagedata);
    }

    public function edit()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('添加角色');

        //面包屑
        $this->runtimePath = array(
            ['url'=> url::action('toptemai_ctl_index@index'),'title' => app::get('toptemai')->_('首页')],
            ['url'=> url::action('toptemai_ctl_account_roles@index'),'title' => app::get('toptemai')->_('角色管理')],
            ['title' => app::get('toptemai')->_('添加角色')],
        );
        ...
        if( input::get('role_id') )
        {
            ...
            $data = app::get('toptemai')->rpcCall('account.shop.roles.get',$params);
            if( $data )
            {
        ...
        return $this->page('toptemai/account/roles/edit.html', $pagedata);
    }
    ...
    public function save()
    {
        ...
        try
        {
            ...
            if( $roleId )
            {
                ...
                $flag = app::get('toptemai')->rpcCall('account.shop.roles.update',$params);
                $msg = '角色修改成功';
            }
            else
            {
                $flag = app::get('toptemai')->rpcCall('account.shop.roles.add',$params);
                $msg = '角色添加成功';
            }
        }
        ...
        $url = url::action('toptemai_ctl_account_roles@index');
    ...
    public function delete()
    {
        ...
        try
        {
            $params['role_id'] = $roleId;
            $params['shop_id'] = $this->shopId;
            app::get('toptemai')->rpcCall('account.shop.roles.delete',$params);
        }
        catch( \LogicException $e )
        {
            $msg = $e->getMessage();
