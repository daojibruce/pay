<?php
class toptemai_ctl_promotion_xydiscount extends toptemai_controller {

    public function list_xydiscount()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('X件Y折管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $xydiscountListData = app::get('toptemai')->rpcCall('promotion.xydiscount.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_xydiscount@list_xydiscount', $filter),
            'current'=>$current,
            'use_app'=>'toptemai',
            'total'=>$total,
            'token'=>$filter['pages'],
        ...
        return $this->page('toptemai/promotion/xydiscount/index.html', $pagedata);
    }


    public function edit_xydiscount()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('添加/编辑X件Y折促销');
        ...
        {
            $pagedata = app::get('toptemai')->rpcCall('promotion.xydiscount.get', $apiData);
        ...
        return $this->page('toptemai/promotion/xydiscount/edit.html', $pagedata);
    }

    //查看X件Y折促销
    public function show_xydiscount(){
        $this->contentHeaderTitle = app::get('toptemai')->_('查看X件Y折促销 ');
        $apiData['xydiscount_id'] = input::get('xydiscount_id');
        $apiData['xydiscount_itemList'] = true;
        if($apiData['xydiscount_id'])
        {
        ...
        return $this->page('toptemai/promotion/xydiscount/show.html',$pagedata);
    }
    ...
    public function save_xydiscount()
    {
        ...
        try
        {
            ...
            {
                // 修改X件Y折促销
                $result = app::get('toptemai')->rpcCall('promotion.xydiscount.update', $apiData);
            }
            else
            {
                // 新添X件Y折促销
                $result = app::get('toptemai')->rpcCall('promotion.xydiscount.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            ...
            if($params['xydiscount_id'])
            {
                $url = url::action('toptemai_ctl_promotion_xydiscount@edit_xydiscount', array('xydiscount_id'=>$params['xydiscount_id']));
            }
            else{
                $url = url::action('toptemai_ctl_promotion_xydiscount@list_xydiscount');
            }
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改xy折促销。xy折促销名称是 '.$apiData['xydiscount_name']);
        $url = url::action('toptemai_ctl_promotion_xydiscount@list_xydiscount');
        $msg = app::get('toptemai')->_('保存X件Y折促销成功');
    ...
    public function submit_approve(){
        $apiData = input::get();
        $xydiscountInfo = app::get('toptemai')->rpcCall('promotion.xydiscount.get',$apiData);
        try{
            if($xydiscountInfo['end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }
            $result = app::get('toptemai')->rpcCall('promotion.xydiscount.approve',$apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    public function delete_xydiscount()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.xydiscount.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除X件Y折促销成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_xydiscount()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.xydiscount.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消X件Y折促销成功');
        return $this->splash('success', $url, $msg, true);
    }
