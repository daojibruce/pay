<?php
class toptemai_ctl_promotion_package extends toptemai_controller {

    public function list_package()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('组合促销管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $packageListData = app::get('toptemai')->rpcCall('promotion.package.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_package@list_package', $filter),
            'current'=>$current,
            'use_app'=>'toptemai',
            'total'=>$total,
            'token'=>$filter['pages'],
        ...
        return $this->page('toptemai/promotion/package/index.html', $pagedata);
    }

    public function edit_package()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('添加/编辑组合促销促销');
        ...
        {
            $pagedata = app::get('toptemai')->rpcCall('promotion.package.get', $apiData);
        ...
        return $this->page('toptemai/promotion/package/edit.html', $pagedata);
    }

    //
    public function show_package(){
        $this->contentHeaderTitle = app::get('toptemai')->_('查看组合促销');
        $apiData['package_id'] = input::get('package_id');
        $apiData['package_itemList'] = true;
        if($apiData['package_id'])
        {
        ...
        return $this->page('toptemai/promotion/package/show.html',$pagedata);
    }
    ...
    public function save_package()
    {
        ...
        try
        {
            ...
            {
                // 修改组合促销促销
                $result = app::get('toptemai')->rpcCall('promotion.package.update', $apiData);
            }
            else
            {
                // 新添组合促销促销
                $result = app::get('toptemai')->rpcCall('promotion.package.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            ...
            if($params['package_id'])
            {
                $url = url::action('toptemai_ctl_promotion_package@edit_package', array('package_id'=>$params['package_id']));
            }
            else{
                $url = url::action('toptemai_ctl_promotion_package@list_package');
            }
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改组合促销。组合促销名称是 '.$apiData['package_name']);
        $url = url::action('toptemai_ctl_promotion_package@list_package');
        $msg = app::get('toptemai')->_('保存组合促销成功');
    ...
    public function submit_approve(){
        $apiData = input::get();
        $packageInfo = app::get('toptemai')->rpcCall('promotion.package.get',$apiData);
        try{
            if($packageInfo['end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }
            $result = app::get('toptemai')->rpcCall('promotion.package.approve',$apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    public function delete_package()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.package.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除组合促销促销成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_package()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.package.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消组合促销成功');
        return $this->splash('success', $url, $msg, true);
    }
