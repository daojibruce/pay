<?php
class toptemai_ctl_promotion_gift extends toptemai_controller{

    //赠品列表
    public function list_gift()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('赠品促销管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $giftListData = app::get('toptemai')->rpcCall('promotion.gift.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_gift@list_gift', $filter),
            'current'=>$current,
            'total'=>$total,
            'use_app'=>'toptemai',
            'token'=>$filter['pages'],
        ...
        return $this->page('toptemai/promotion/gift/index.html', $pagedata);
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('查看赠品促销');
        $apiData['gift_id'] = input::get('gift_id');
        $apiData['gift_itemList'] = true;
        if($apiData['gift_id']){
            $pagedata = app::get('toptemai')->rpcCall('promotion.gift.get',$apiData);
        ...
        //$pagedata['shopCatList'] = app::get('toptemai')->rpcCall('shop.authorize.cat',array('shop_id'=>$shopid));
        $pagedata['ac'] = input::get('ac', '');
        $pagedata['image_default_id'] = kernel::single('image_data_image')->getImageSetting('item');
        return $this->page('toptemai/promotion/gift/show.html',$pagedata);
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('新添/编辑赠品促销');
        $apiData['gift_id'] = input::get('gift_id');
        $apiData['gift_itemList'] = true;
        ...
        return $this->page('toptemai/promotion/gift/edit.html', $pagedata);
    }

    //提交审核
    public function submit_approve(){
        $apiData = input::get();
        $xydiscountInfo = app::get('toptemai')->rpcCall('promotion.gift.get',$apiData);
        try{
            if($xydiscountInfo['end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }
            $result = app::get('toptemai')->rpcCall('promotion.gift.approve',$apiData);
        }
        ...
        $url = url::action('toptemai_ctl_promotion_gift@list_gift');
    ...
    public function save_gift()
    {
        ...
        try
        {
            if($params['gift_id'])
            {
                ...
                // 修改赠品促销
                $result = app::get('toptemai')->rpcCall('promotion.gift.update', $apiData);
            }
            else
            {
                // 新添加赠品促销
                $result = app::get('toptemai')->rpcCall('promotion.gift.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
            $url = url::action('toptemai_ctl_promotion_gift@edit_gift', array('gift_id'=>$params['gift_id']));
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改赠品促销。赠品促销名称是 '.$apiData['gift_name']);
        $url = url::action('toptemai_ctl_promotion_gift@list_gift');
        $msg = app::get('toptemai')->_('保存赠品促销成功');
        return $this->splash('success',$url,$msg,true);
    }
    ...
    public function delete_gift()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.gift.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除赠品促销成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_gift()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.gift.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消赠品促销成功');
        return $this->splash('success', $url, $msg, true);
    }
}
