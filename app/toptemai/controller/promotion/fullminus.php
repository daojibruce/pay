<?php
class toptemai_ctl_promotion_fullminus extends toptemai_controller {

    public function list_fullminus()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('满减管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $fullminusListData = app::get('toptemai')->rpcCall('promotion.fullminus.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_fullminus@list_fullminus', $filter),
            'current'=>$current,
            'use_app'=>'toptemai',
            'total'=>$total,
            'token'=>$filter['pages'],
        ...
        return $this->page('toptemai/promotion/fullminus/index.html', $pagedata);
    }

    public function edit_fullminus()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('添加/编辑满减促销');
        ...
        {
            $pagedata = app::get('toptemai')->rpcCall('promotion.fullminus.get', $apiData);
        ...
        return $this->page('toptemai/promotion/fullminus/edit.html', $pagedata);
    }
    //查看满减 by wujie
    public function show_fullminus()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('查看满减促销');
        $apiData['fullminus_id'] = input::get('fullminus_id');
        $apiData['fullminus_itemList'] = true;
        if($apiData['fullminus_id'])
        {
        ...
        $pagedata['condition_value'] = $this->condition($pagedata['condition_value']);
        $pagedata['ac'] = input::get('ac', '');
        return $this->page('toptemai/promotion/fullminus/show.html',$pagedata);
    }
    ...
    public function save_fullminus()
    {
        ...
        try
        {
            ...
            {
                // 修改满减促销
                $result = app::get('toptemai')->rpcCall('promotion.fullminus.update', $apiData);
            }
            else
            {
                // 新添满减促销
                $result = app::get('toptemai')->rpcCall('promotion.fullminus.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            ...
            if($params['fullminus_id'])
            {
                $url = url::action('toptemai_ctl_promotion_fullminus@edit_fullminus', array('fullminus_id'=>$params['fullminus_id']));
            }
            else{
                $url = url::action('toptemai_ctl_promotion_fullminus@list_fullminus');
            }
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改满减促销。满减促销名称是 '.$apiData['fullminus_name']);
        $url = url::action('toptemai_ctl_promotion_fullminus@list_fullminus');
        $msg = app::get('toptemai')->_('保存满减促销成功');
    ...
    public function submit_approve(){
        ...
        try{
            ...
            $fullminusInfo = app::get('toptemai')->rpcCall('promotion.fullminus.get',$apiData);
            if($fullminusInfo['end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }

            $result = app::get('toptemai')->rpcCall('promotion.fullminus.approve',$apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    public function delete_fullminus()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.fullminus.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除满减促销成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_fullminus()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.fullminus.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消满减促销成功');
        return $this->splash('success', $url, $msg, true);
    }
