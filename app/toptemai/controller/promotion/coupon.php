<?php
class toptemai_ctl_promotion_coupon extends toptemai_controller {

    public function list_coupon()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('优惠券管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $couponListData = app::get('toptemai')->rpcCall('promotion.coupon.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_coupon@list_coupon', $filter),
            'current'=>$current,
            'total'=>$total,
            'use_app'=>'toptemai',
            'token'=>$filter['pages'],
        ...
        return $this->page('toptemai/promotion/coupon/index.html', $pagedata);
    }

    public function edit_coupon()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('新添/编辑优惠券');
        ...
        {
            $pagedata = app::get('toptemai')->rpcCall('promotion.coupon.get', $apiData);
        ...
        return $this->page('toptemai/promotion/coupon/edit.html', $pagedata);
    }

    //查看优惠券
    public function show_coupon(){
        $this->contentHeaderTitle = app::get('toptemai')->_('查看优惠券');
        $apiData['coupon_id'] = input::get('coupon_id');
        $apiData['coupon_itemList'] = true;
        if($apiData['coupon_id']){
            $pagedata = app::get('toptemai')->rpcCall('promotion.coupon.get',$apiData);
        ...
        return $this->page('toptemai/promotion/coupon/show.html',$pagedata);
    }
    ...
    public function save_coupon()
    {
        ...
        try
        {
            ...
            {
                // 修改优惠券
                $result = app::get('toptemai')->rpcCall('promotion.coupon.update', $apiData);
            }
            else
            {
                // 新添优惠券
                $result = app::get('toptemai')->rpcCall('promotion.coupon.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
            $url = url::action('toptemai_ctl_promotion_coupon@edit_coupon', array('coupon_id'=>$params['coupon_id']));
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改优惠券。优惠券名称是 '.$apiData['coupon_name']);
        $url = url::action('toptemai_ctl_promotion_coupon@list_coupon');
        $msg = app::get('toptemai')->_('保存优惠券成功');
    ...
    public function submit_approve(){
        $apiData = input::get();
        try{
            $couponInfo = app::get('toptemai')->rpcCall('promotion.coupon.get',$apiData);
            if($couponInfo['cansend_end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }
            $result = app::get('toptemai')->rpcCall('promotion.coupon.approve',$apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    public function delete_coupon()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.coupon.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除优惠券成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_coupon()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.coupon.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消优惠券促销成功');
        return $this->splash('success', $url, $msg, true);
    }
