...
class toptemai_ctl_promotion_fulldiscount extends toptemai_controller {

    public function list_fulldiscount()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('满折管理');
        $filter = input::get();
        if(!$filter['pages'])
        {
            $filter['pages'] = 1;
        ...
        $fulldiscountListData = app::get('toptemai')->rpcCall('promotion.fulldiscount.list', $params,'seller');
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_promotion_fulldiscount@list_fulldiscount', $filter),
            'current'=>$current,
            'use_app'=>'toptemai',
            'total'=>$total,
            'token'=>$filter['pages'],
        ...
        $gradeList = app::get('toptemai')->rpcCall('user.grade.list');
        ...
        return $this->page('toptemai/promotion/fulldiscount/index.html', $pagedata);
    }


    public function edit_fulldiscount()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('添加/编辑满折促销');
        ...
        {
            $pagedata = app::get('toptemai')->rpcCall('promotion.fulldiscount.get', $apiData);
        ...
        $pagedata['gradeList'] = app::get('toptemai')->rpcCall('user.grade.list');
        ...
        return $this->page('toptemai/promotion/fulldiscount/edit.html', $pagedata);
    }

    //查看满折
    public function show_fulldiscount()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('查看满折');
        $apiData['fulldiscount_id'] = input::get('fulldiscount_id');
        $apiData['fulldiscount_itemList'] = true;
        if($apiData['fulldiscount_id'])
        {
        ...
        $pagedata['condition_value'] = $this->condition($pagedata['condition_value']);
        $pagedata['ac'] = input::get('ac', '');
        return $this->page('toptemai/promotion/fulldiscount/show.html',$pagedata);
    }
    ...
    public function save_fulldiscount()
    {
        ...
        try
        {
            ...
            {
                // 修改满折促销
                $result = app::get('toptemai')->rpcCall('promotion.fulldiscount.update', $apiData);
            }
            else
            {
                // 新添满折促销
                $result = app::get('toptemai')->rpcCall('promotion.fulldiscount.add', $apiData);
            }
        }
        catch(\LogicException $e)
        {
            ...
            if($params['fulldiscount_id'])
            {
                $url = url::action('toptemai_ctl_promotion_fulldiscount@edit_fulldiscount', array('fulldiscount_id'=>$params['fulldiscount_id']));
            }
            else{
                $url = url::action('toptemai_ctl_promotion_fulldiscount@list_fulldiscount');
            }
            return $this->splash('error',$url,$msg,true);
        }
        $this->sellerlog('添加/修改满折促销。满折促销名称是 '.$apiData['fulldiscount_name']);
        $url = url::action('toptemai_ctl_promotion_fulldiscount@list_fulldiscount');
        $msg = app::get('toptemai')->_('保存满折促销成功');
    ...
    public function submit_approve(){
        $apiData = input::get();
        try{
            $fulldiscountInfo = app::get('toptemai')->rpcCall('promotion.fulldiscount.get',$apiData);
            if($fulldiscountInfo['end_time'] <= time()){
                throw new \LogicException('您的活动已过期，无法提交审核!');
            }
            $result = app::get('toptemai')->rpcCall('promotion.fulldiscount.approve',$apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
    ...
    public function delete_fulldiscount()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.fulldiscount.delete', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('删除满折促销成功');
        return $this->splash('success', $url, $msg, true);
    }
    ...
    public function cancel_fulldiscount()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('promotion.fulldiscount.cancel', $apiData);
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
        ...
        $msg = app::get('toptemai')->_('取消满折促销成功');
        return $this->splash('success', $url, $msg, true);
    }
