 ...
 */
class toptemai_ctl_item_importexport extends toptemai_controller {
    ...
    {
        try {
            $objExport = kernel::single('toptemai_item_export', $this->shopId);
            $exportFilter = $this->__exportFilter();
            $this->sellerlog(app::get('toptemai')->_('导出商品'));
            // 导出
    ...
    public function importView()
    {
        ...
        // 获取当前店铺可用的三级分类
        try {
            $objImport = kernel::single('toptemai_item_import', $this->shopId);
            $pagedata['lv3Cat'] = $objImport->getLv3CatWithLv2();
        } catch (Exception $e) {
            return $this->splash('error', null, $e->getMessage());
        }
        
        $data['html'] = view::make('toptemai/item/import.html', $pagedata)->render();
        $data['success'] = true;
    ...
    public function downLoadImportTmpl()
    {
        ...
        try {
            ...
            if(!$catId)
            {
                throw new LogicException(app::get('toptemai')->_('参数错误'));
            }
            $objImport = kernel::single('toptemai_item_import', $this->shopId);
    ...
    public function import()
    {
        try {
            ...
            if(!$fileInfo)
            {
                throw new LogicException(app::get('toptemai')->_('请上传文件'));
            }
        ...
        $this->sellerlog(app::get('toptemai')->_('导入商品'));
        $url = request::server('HTTP_REFERER');
        return $this->splash('success', $url, app::get('toptemai')->_('商品导入成功'));
    }
    ...
    private function __exportFilter()
    {
        ...
        {
            throw new LogicException(app::get('toptemai')->_('导出参数错误'));
        }
        $filter = input::get();
        $params = [];
