<?php
class toptemai_ctl_rate_appeal extends toptemai_controller {
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('申诉管理-申诉列表');
        $pagedata = $this->__searchAppealData();
        return $this->page('toptemai/rate/appeal/index.html', $pagedata);
    }

    public function search()
    {
        $pagedata = $this->__searchAppealData();
        return view::make('toptemai/rate/appeal/list.html', $pagedata);
    }

    //申诉详情
    public function appeaInfo()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('申诉管理-申诉详情');
        ...
        $data = app::get('toptemai')->rpcCall('rate.get', $params,'seller');
        ...
        return $this->page('toptemai/rate/appeal/info.html', $pagedata);
    }
    ...
    private function __searchAppealData()
    {
        ...
        $data = app::get('toptemai')->rpcCall('rate.list.get', $params,'seller');
        ...
        if( $userId )
        {
            $pagedata['userName'] = app::get('toptemai')->rpcCall('user.get.account.name', ['user_id' => implode(',', $userId)], 'seller');
        }
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_rate_appeal@appealList',$filter),
            'current'=>$current,
            'total'=>$total,
            'use_app'=>'toptemai',
            'token'=>$filter['pages'],
    ...
    public function appeal()
    {
        ...
        try {
            $flag = app::get('toptemai')->rpcCall('rate.appeal.add', $params,'seller');
            $status = $flag ? 'success' : 'error';
            $msg = $flag ? app::get('toptemai')->_('申诉已提交，请耐心等待，我们将会在10个工作日内给予审核回复，谢谢！ ') : app::get('toptemai')->_('申诉提交失败，请重新再试');
        } catch (Exception $e) {
            $msg = $e->getMessage();
            $status = 'error';
        }
        ...
        if( $params['is_again'] )
        {
            $url = url::action('toptemai_ctl_rate_appeal@appeaInfo',['rate_id'=>$params['rate_id']]);
        }
        else
        {
            $url = url::action('toptemai_ctl_rate@detail',['rate_id'=>$params['rate_id']]);
        }
