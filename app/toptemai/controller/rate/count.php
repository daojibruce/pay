...
class toptemai_ctl_rate_count extends toptemai_controller {


    //评价列表
    public function index()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('评价管理-商家评价概况');

        $params['shop_id'] = $this->shopId;
        $params['catDsrDiff'] = true;
        $params['countNum'] = true;
        $dsrData = app::get('toptemai')->rpcCall('rate.dsr.get', $params);
        if( !$dsrData )
        {
        ...
        $pagedata['count'] = app::get('toptemai')->rpcCall('rate.count',array('shop_id' => $this->shopId));

        return $this->page('toptemai/rate/count.html', $pagedata);
    }
