...
class toptemai_ctl_selector_item extends toptemai_controller {
    public $limit = 14;
    ...
    public function loadSelectGoodsModal()
    {
        ...
        $pagedata['shopCatList'] = false;#app::get('toptemai')->rpcCall('shop.authorize.cat',array('user_id'=>$this->userId));
        return view::make('toptemai/selector/item/index.html', $pagedata);
    }

    public function formatSelectedGoodsRow()
    {
        $itemIds = input::get('item_id');
        is_array($itemIds) || $itemIds = array($itemIds);
        $textcol = input::get('textcol');
        $ac = input::get('ac');
        $extendView = input::get('view');
        $itemSku = input::get('item_sku');

        $itemIdsChunk = array_chunk($itemIds, 40);
        $itemsList = array();
        foreach( $itemIdsChunk as $value )
        {
            $searchParams = array(
                ...
                'extends' => ['brand_id' => true , 'sku_id' => false , 'sku' => true , 'item_store' => false , 'item_status' => false],
            );
            $itemsListData = app::get('syspromotion')->rpcCall('tmitem.list.get',$searchParams);
        ...
        return view::make('toptemai/selector/item/input-row.html', $pagedata);
    }
    ...
    public function searchItem($json=true)
    {
        ...
        if($catId)
        {
            $searchParams['cat_id'] = app::get('toptemai')->rpcCall('category.cat.get.leafCatId',array('cat_id'=>intval($catId)));
        }
        if(trim($brandName) && trim($brandName) != 'undefined')
        {
            $searchBrandParams = array('brand_name'=>trim($brandName),'fields'=>'brand_id');
            $brand = app::get('toptemai')->rpcCall('category.brand.get.list', $searchBrandParams);
            if($brand)
            {
            ...
            {
                return view::make('toptemai/selector/item/list.html', $pagedata);
            }
        }

        $searchParams['fields'] = 'item_id,title,image_default_id,price,brand_id';
        $itemsList = app::get('toptemai')->rpcCall('temai.item.search', $searchParams);
        ...
        $filter['pages'] = time();
        $pagers = array(
            'link' => url::action('toptemai_ctl_selector_item@searchItem', $filter),
            'current' => $pages,
            'use_app' => 'toptemai',
            'total' => $totalPage,
            'token' => time(),
    ...
    public function getSkuByItemId()
    {
        ...
        $searchParams['user_id'] = $this->userId;
        $skusList = app::get('toptemai')->rpcCall('sku.search', $searchParams);
        $pagedata['_input']['skusList'] = $skusList['list'];
        $pagedata['image_default_id'] = kernel::single('image_data_image')->getImageSetting('item');
        return view::make('toptemai/selector/item/skus.html', $pagedata);
    }
    ...
    public function showSkuByitemId()
    {
        ...
        return view::make('toptemai/selector/item/show_skus.html', $pagedata);
    }
}
