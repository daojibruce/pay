...
class toptemai_ctl_shop_decorate extends toptemai_controller {
    ...
    public function index()
    {
        ...
        if( input::get('show',false) )
        {
            ...
            try {
                ...
                $html = "toptemai/shop/decorate/widgets/{$params[0]}/{$params[1]}.html";
            } catch (Exception $e) {
                return redirect::action('toptemai_ctl_shop_decorate@index');
            }
        ...
        return $this->page('toptemai/shop/decorate.html', $pagedata);
    }
    ...
    public function dialog()
    {
        ...
        $this->runtimePath = array(
            ['url'=> url::action('toptemai_ctl_shop_decorate@index'),'title' => app::get('toptemai')->_('店铺装修')],
            ['url'=> url::action('toptemai_ctl_shop_decorate@dialog',['widgets'=>$widgetsName,'dialog'=>'default']),'title' => $this->headerTitle[$widgetsName]['default']],
        );
        ...
        return $this->page("toptemai/shop/decorate/widgets/{$widgetsName}/{$dialogName}.html", $pagedata);
    }
    ...
    public function save()
    {
        ...
        if( $showdialog )
        {
            $url = url::action('toptemai_ctl_shop_decorate@dialog',array('widgets'=>input::get('widgets'),'dialog'=>$showdialog));
        }
        else
        {
            $url = url::action('toptemai_ctl_shop_decorate@index');
        }
    ...
    private function _checkParams($params)
    {
        if(isset($params['data']) && is_array($params['data']))
        {
            foreach ($params['data'] as $value)
            {
                if($value['order_sort'])
                {
                    ...
                    {
                        throw new LogicException(app::get('toptemai')->_('排序序号必须为正整数'));
                    }
                }
            }
        }
        ...
        if(isset($params['num']))
        {
            ...
            {
                throw new LogicException(app::get('toptemai')->_('数量必须为正整数'));
            }
        }
