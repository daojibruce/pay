<?php
class toptemai_ctl_trade_flow extends toptemai_controller{
    /**
     * 变更物流信息
     * @params null
     * @return null
      ...
      //$requestData['dlytmpl_id'] = input::get('dlytmpl_id');
        ...
        try{
            ...
            if(!$tradeInfo['status']= 'WAIT_BUYER_CONFIRM_GOODS')
                throw new Exception(app::get('toptemai')->_('只能修改已发货待收货订单的物流信息'));

            app::get('toptemai')->rpcCall('delivery.updateLogistic', $requestData);
        }catch(Exception $e){
            return $this->splash('error',null, $e->getMessage(), true);
        }

        $url = url::action('toptemai_ctl_trade_detail@index', ['tid'=>$tid]);
    ...
    public function dodelivery()
    {
        ...
        try
        {
            app::get('toptemai')->rpcCall('trade.delivery',$sdf);
        }
        catch (Exception $e)
        {
            return $this->splash('error',null, $e->getMessage(), true);
        ...
        $url = url::action('toptemai_ctl_trade_list@index', ['useSessionFilter'=>true]);
    ...
    public function goDelivery()
    {
        ...
        $this->runtimePath = array(
            ...
            ['title' => app::get('toptemai')->_('订单发货')],
        );
        $this->contentHeaderTitle = app::get('toptemai')->_('订单发货');

        $tid = input::get('tid');
        if(!$tid)
        {
            header('Content-Type:application/json; charset=utf-8');
            echo '{error:"'.app::get('toptemai')->_("订单号传递出错.").'",_:null}';exit;
        }
        $params['tid'] = $tid;
        ...
        $dlycorp = app::get('toptemai')->rpcCall('shop.dlycorp.getlist',['shop_id'=>$this->shopId]);
        $pagedata['dlycorp'] = $dlycorp['list'];

        return $this->page('toptemai/trade/godelivery.html', $pagedata);
    }
}
