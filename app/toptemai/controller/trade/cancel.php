<?php
class toptemai_ctl_trade_cancel extends toptemai_controller {

    public $limit = 10;

    public function index()
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('订单取消列表');
        $apiParams['temai_user_id'] = $this->userId;
        if( input::get('tid') )
        {
        ...
        $data = app::get('toptemai')->rpcCall('temai.trade.cancel.list.get', $apiParams);
        ...
        return $this->page('toptemai/trade/cancel/list.html', $pagedata);
    }
    ...
    public function ajaxSearch()
    {
        ...
        return view::make('toptemai/trade/cancel/item.html', $pagedata);
    }
    ...
    private function __pager($total, $current)
    {
        ...
        $pagers = array(
            'link'=>url::action('toptemai_ctl_trade_cancel@ajaxSearch',$filter),
            'current'=>$current,
            'total'=>$totalPage,
            'use_app'=>'toptemai',
            'token'=>time(),
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('订单取消详情');
        ...
        $this->runtimePath = array(
            ...
            ['url'=> url::action('toptemai_ctl_trade_cancel@index'),'title' => app::get('toptemai')->_('订单取消列表')],
            ['title' => app::get('toptemai')->_('订单取消详情')],
        );

        $cancelId = input::get('cancel_id');
        try{
            $data = app::get('topc')->rpcCall('temai.trade.cancel.get',['temai_user_id'=>$this->userId,'cancel_id'=>$cancelId]);
        }catch(Exception $e){
    	    return $this->page('toptemai/trade/cancel/detail.html',$pagedata);
        }
        ...
        {
            $pagedata['logi'] = app::get('toptemai')->rpcCall('delivery.get',array('tid'=>$tradeInfo['tid']));
        }
        $pagedata['image_default_id'] = kernel::single('image_data_image')->getImageSetting('item');
    ...
    public function shopCheckCancel()
    {
        ...
        try{
            app::get('toptemai')->rpcCall('trade.cancel.shop.check',$params);
        }
        catch( LogicException $e ){
            return $this->splash('error',null, $e->getMessage(), true);
        }
        $this->sellerlog('处理订单取消申请。申请ID是'.$params['cancel_id']);
        $url = url::action('toptemai_ctl_trade_cancel@detail',['cancel_id'=>$params['cancel_id']]);
