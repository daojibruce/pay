<?php
class toptemai_ctl_rate extends toptemai_controller {
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('评价管理-评价列表');
        $pagedata = $this->__searchRateData();
        return $this->page('toptemai/rate/list.html', $pagedata);
    }

    public function search()
    {
        $pagedata = $this->__searchRateData();
        return view::make('toptemai/rate/list_item.html', $pagedata);
    }
    ...
    private function __searchRateData()
    {
        ...
        $pagedata['pagers'] = array(
            'link'=>url::action('toptemai_ctl_rate@index',$filter),
            'current'=>$current,
            'use_app'=>'toptemai',
            'total'=>$total,
            'token'=>$filter['pages'],
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('评价管理-评价详情');
        ...
        if( empty($data) )
        {
            return redirect::action('toptemai_ctl_rate@index');
        }
        ...
        $pagedata['rate'] = $data;
        return $this->page('toptemai/rate/detail.html', $pagedata);
    }
    ...
    public function reply()
    {
        ...
        try {
            if( $replyType == 'add')
            {
                $result = app::get('toptemai')->rpcCall('rate.reply.add', $params,'seller');
            }
            else
            {
                $params['shop_id'] = $this->shopId;
                $result = app::get('toptemai')->rpcCall('rate.append.reply', $params);
            }

            $status = $result ? 'success' : 'error';
            $msg = $result ? app::get('toptemai')->_('回复成功') : app::get('toptemai')->_('回复失败');
        } catch (Exception $e) {
            $msg = $e->getMessage();
            $status = 'error';
        }
