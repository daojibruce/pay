<?php
class toptemai_ctl_aftersales extends toptemai_controller {
    ...
    public function index()
    {
        ...
        $pagedata['refund_type'] = array(
            'ONLY_REFUND' => app::get('toptemai')->_('仅退款'),
            'REFUND_GOODS' => app::get('toptemai')->_('退货退款'),
            'EXCHANGING_GOODS' => app::get('toptemai')->_('换货'),
        );
        $pagedata['progress'] = array(
            '0' => app::get('toptemai')->_('等待审核'),
            '1' => app::get('toptemai')->_('等待买家回寄'),
            '2' => app::get('toptemai')->_('待确认收货'),
            '5' => app::get('toptemai')->_('商家已收货'),
            '8' => app::get('toptemai')->_('等待平台退款'),
            '3-4-6-7' => app::get('toptemai')->_('已完成'),//换货的时候可以直接在商家处理结束
        );
        
        $pagedata['complaints'] = array(
                'NOT_COMPLAINTS' => app::get('toptemai')->_('未投诉'),
                'WAIT_SYS_AGREE' => app::get('toptemai')->_('买家已发起投诉'),
                'FINISHED' => app::get('toptemai')->_('投诉受理'),
                'BUYER_CLOSED' => app::get('toptemai')->_('买家撤销投诉'),
                'CLOSED' => app::get('toptemai')->_('投诉驳回'),
        );

        //获取默认图片信息
        $pagedata['defaultImageId']= kernel::single('image_data_image')->getImageSetting('item');

        return $this->page('toptemai/aftersales/list.html', $pagedata);
    }
    ...
    private function __searchListData()
    {
        ...
        try{
            $result = app::get('toptemai')->rpcCall('aftersales.list.get', $params, 'seller');
        ...
        $data['pagers'] = array(
            'link'=>url::action('toptemai_ctl_aftersales@search',$filter),
            'current'=>$current,
            'total'=>$total,
            'use_app'=>'shop',
            'token'=>$filter['pages'],
    ...
    public function detail()
    {
        ...
        try{
            $result = app::get('toptemai')->rpcCall('aftersales.get', $params,'seller');
        }
        catch(Exception $e)
        {
            redirect::action('toptemai_ctl_aftersales@index')->send();exit;
        }
        ...
        if( $result['user_id'] )
        {
             $userName = app::get('toptemai')->rpcCall('user.get.account.name', ['user_id' => $result['user_id']], 'seller');
        ...
        //快递公司代码
        $corpData = app::get('toptemai')->rpcCall('shop.dlycorp.getlist',['shop_id'=>$this->shopId]);
        ...
        {
            $refunds = app::get('toptemai')->rpcCall('aftersales.refundapply.list.get',['fields'=>'status,total_price','oid'=>$result['oid']]);
            $refunds = $refunds['list'][0];
            $pagedata['refunds'] = $refunds;
        }

        $pagedata['tracking'] = app::get('syslogistics')->getConf('syslogistics.order.tracking');
        return $this->page('toptemai/aftersales/detail.html', $pagedata);
    }
    ...
    public function search()
    {
        ...
        $pagedata['complaints'] = array(
                'NOT_COMPLAINTS' => app::get('toptemai')->_('未投诉'),
                'WAIT_SYS_AGREE' => app::get('toptemai')->_('买家已投诉'),
                'FINISHED' => app::get('toptemai')->_('投诉受理'),
                'BUYER_CLOSED' => app::get('toptemai')->_('买家撤销投诉'),
                'CLOSED' => app::get('toptemai')->_('投诉驳回'),
        );
        ...
        return view::make('toptemai/aftersales/item.html', $pagedata);
    }
    ...
    public function sendConfirm()
    {
        ...
        try
        {
            $result = app::get('toptemai')->rpcCall('aftersales.send.confirm',$postdata,'seller');
        }
        ...
        $url = url::action('toptemai_ctl_aftersales@detail', array('bn'=>$postdata['aftersales_bn']));
        $msg = '操作成功';
    ...
    public function verification()
    {
        ...
        try
        {
            $result = app::get('toptemai')->rpcCall('aftersales.check',$postdata,'seller');
        }
        catch(\LogicException $e)
        {
            $msg = $e->getMessage();
