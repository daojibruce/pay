 ...
 * @brief
 */
class toptemai_ctl_sku extends toptemai_controller{

    public function getNatureProps()
    {
        $catId = intval(input::get('cat_id'));
        $itemId = intval(input::get('item_id'));
        $pagedata['nature_props'] = app::get('toptemai')->rpcCall('category.catprovalue.get',array('cat_id'=>$catId,'type'=>'nature'));
        if($itemId)
        {
            $item_nature_props = app::get('toptemai')->rpcCall('item.get.nature.prop',array('item_id'=>$itemId));
            ...
            foreach ($pagedata['nature_props'] as $k => $v)
            {
                if(array_key_exists($v['prop_id'], $isSelectionProp))
                {
                    $pagedata['nature_props'][$k]['selected_prop_value_id'] = $isSelectionProp[$v['prop_id']];
                }
            }
        }
        ...
        foreach ($pagedata['nature_props'] as $key => $val) {
            if ($val['show'] == 'text') {
                if ($val['selected_prop_value_id'] > 0) {
                    foreach ($val['prop_value'] as $id => $v) {
                        if ($val['selected_prop_value_id'] != $v['prop_value_id']) {
                            unset($pagedata['nature_props'][$key]['prop_value'][$id]);
                        }
                    }
                } else {
                    unset($pagedata['nature_props'][$key]['prop_value']);
                }
            }
        }
        return view::make('toptemai/item/props/nature.html',$pagedata);
    }
    ...
    public function getSpecProps()
    {
        ...
        {
            $specProp = app::get('toptemai')->rpcCall('category.catprovalue.get',array('cat_id'=>$catId,'type'=>'spec'));
        ...
        return view::make('toptemai/item/props/spec.html', $pagedata);
    }
    ...
    public function getParams()
    {
        ...
        $catInfo = app::get('toptemai')->rpcCall('category.cat.get.info',array('cat_id'=>$catId,'fields'=>'params'));
        if($itemId)
        {
            $itemInfo = app::get('toptemai')->rpcCall('item.get',array('item_id'=>$itemId,'fields'=>'item_id, cat_id, params'));
            if($itemInfo['params'])
            {
        ...
        return view::make('toptemai/item/params.html', $pagedata);
    }

    function getSpecPropsByItemId($itemId)
    {
        $itemInfo = app::get('toptemai')->rpcCall('item.get',array('item_id'=>$itemId,'fields'=>'item_id,spec_desc'));
        ...
        if( $itemInfo['spec_desc'] && is_array( $itemInfo['spec_desc'] ) )
        {
            ...
            $propsList = app::get('toptemai')->rpcCall('category.prop.list',array('prop_id'=>$propIds));
    ...
    public function set_spec($catId, $params_spec){
        ...
        if($params_spec)
        {
            ...
            $specifications = app::get('toptemai')->rpcCall('category.catprovalue.get',array('cat_id'=>$catId,'prop_id'=>$prop_id,'type'=>'spec'));
        }
        else
        {
            return false;
    ...
    public function getProducts($itemId=0){
        ...
        $productData = app::get('toptemai')->rpcCall('item.sku.list',array('item_id'=>$itemId));
...
/*-----------------------以上为编辑货品显示数据处理函数-----------------------------*/
    ...
    private function _get_active_products($item_id)
    {
        ...
        $ordersItemsData = app::get('toptemai')->rpcCall('trade.order.list.get',$params);
        ...
        $params['fields'] = "oid,sku_id";
        $ordersData = app::get('toptemai')->rpcCall('trade.order.list.get',$params);
