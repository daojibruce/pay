...
class toptemai_ctl_passport extends toptemai_controller {

    //简单的登录页面
    public function simpleSignin()
    {
        return view::make('toptemai/passport/simpleSignin.html');
    }
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('企业账号登录');
        ...
        $pagedata['backgroundImgUrl'] = $pagedata['backgroundImgUrl'] ?: app::get('toptemai')->res_url . '/images/bj_01.jpg';
        $pagedata['openid'] = input::get('openid');
        $pagedata['oauthcode'] = input::get('oauthcode');

        return $this->page('toptemai/passport/signin.html',$pagedata);
    }
    ...
    public function login()
    {
        if( pamAccount::isEnableVcode('sysshop') )
        {
            ...
            {
                $msg = app::get('toptemai')->_('图片验证码错误') ;
                $url = url::action('toptemai_ctl_passport@signin');
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('企业账号注册');
        $this->set_tmpl('pwdfind');
        $pagedata['license'] = app::get('sysshop')->getConf('sysshop.register.setting_sysshop_license');
        return $this->page('toptemai/passport/signup.html', $pagedata);
    }
...
//        }
    ...
    {
        pamAccount::logout();
        return redirect::action('toptemai_ctl_passport@signin');
    }
    ...
    {
        return view::make('toptemai/passport/update.html');
    }
    ...
    public function updatepwd()
    {
        ...
        $url = url::action('toptemai_ctl_passport@signin');
        $msg = app::get('toptemai')->_('修改成功,请重新登陆');
        pamAccount::logout();
