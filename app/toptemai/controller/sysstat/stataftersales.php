...
class toptemai_ctl_sysstat_stataftersales extends toptemai_controller
{
	public function index()
	{
		...
		//获取数据
		$data = app::get('toptemai')->rpcCall('sysstat.data.get',$notAll,'seller');
		...
		$refundToptenItem = app::get('toptemai')->rpcCall('sysstat.data.get',$refundParams,'seller');
		...
		$changingToptenItem = app::get('toptemai')->rpcCall('sysstat.data.get',$changingParams,'seller');
		$pagedata['changingToptenItem'] = $changingToptenItem['sysTrade'];

		//获取页面显示的时间
		$pagetime = app::get('toptemai')->rpcCall('sysstat.datatime.get',$all,'seller');

		$pagedata['sendtype'] = $type;
		$pagedata['pagetime'] = $pagetime;
		$pagedata['sysTradeData'] = $data['sysTradeData'];
		$this->contentHeaderTitle = app::get('toptemai')->_('运营报表-问题订单分析');

		return $this->page('toptemai/sysstat/stataftersales.html', $pagedata);
	}
	...
	public function ajaxTrade()
	{
		...
		$datas =  app::get('toptemai')->rpcCall('sysstat.data.get',$all,'seller');
