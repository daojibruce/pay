...
class toptemai_ctl_clearing_settlement extends toptemai_controller
{
    public $limit = 10;
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('商家结算汇总');
        ...
        try{
            $settlement_list = app::get('toptemai')->rpcCall('temai.clearing.getList',$filter);
        }
        ...
        $postSend['page'] = time();
        $link = url::action('toptemai_ctl_clearing_settlement@index',$postSend);
        $pagedata['pagers'] = $this->__pagers($pagedata['count'],$page,$limit,$link);

        return $this->page('toptemai/clearing/settlement.html', $pagedata);
    }
    ...
    {
        $this->contentHeaderTitle = app::get('toptemai')->_('商家结算明细');
        ...
        $result = app::get('toptemai')->rpcCall('temai.clearing.detail.getlist',$filter);
        ...
        $limit = $this->limit;
        $postSend['page'] = time();
        $link = url::action('toptemai_ctl_clearing_settlement@detail',$postSend);
        $pagedata['pagers'] = $this->__pagers($pagedata['count'],$page,$limit,$link);
        return $this->page('toptemai/clearing/settlement_detail.html', $pagedata);
    }
    ...
    private function __getTradePaymentType($list)
    {
        ...
        $params['status'] = 'succ';
        $data = app::get('toptemai')->rpcCall('trade.payment.list', $params);
