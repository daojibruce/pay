 ...
 * @brief 商家入驻申请及查看页面
 */
class toptemai_ctl_enterapply extends toptemai_controller{
    ...
    {
        $result = app::get('toptemai')->rpcCall('shop.get.enterapply',array('seller_id'=>$this->sellerId,'fields'=>'seller_id'));
        if($result)
        {
            return redirect::action('toptemai_ctl_enterapply@checkPlan');
        }
        else
        {
            ...
            if($_POST['license'] != "true")
            {
                ...
                return $this->page('toptemai/enterapply/premise.html', $pagedata);
            }
            else
            {
                $this->contentHeaderTitle = '填写入驻申请资料';
                return $this->page('toptemai/enterapply/apply.html', $pagedata);
            }
        }
    }
    ...
    {
        $datalist = app::get('toptemai')->rpcCall('shop.get.enterapply',array('seller_id'=>$this->sellerId,'fields'=>'enterlog,status'));
        $pagedata['logdata'] = unserialize($datalist['enterlog']);
        $pagedata['data'] = $datalist;
        $this->set_tmpl('commonpage');
        $this->contentHeaderTitle = '查看入驻申请进度';
        return $this->page('toptemai/enterapply/checkPlan.html', $pagedata);
    }
