 ...
 * 检查该帐号是否能够进行入住申请
 */
class toptemai_middleware_register
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $datalist = app::get('toptemai')->rpcCall('shop.get.enterapply',array('seller_id'=>$sellerId,'fields'=>'status'));
        $status = $datalist['status'];

        if($status == 'active' && !in_array($method, ['enterProcessWaiteExamine']))
            return redirect::action('toptemai_ctl_register@enterProcessWaiteExamine');

        if($status == 'successful' && !in_array($method, ['enterProcessWaiteAward']))
            return redirect::action('toptemai_ctl_register@enterProcessWaiteAward');
        ...
        if($status == 'finish')
            return redirect::action('toptemai_ctl_index@index');
