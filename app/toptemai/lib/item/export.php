...
class toptemai_item_export {
    ...
    public function export($params)
    {
        ...
        if(isset($params['approve_status']) && $params['approve_status'] == 'oversku')
        {
            ...
            if($isSearch)
            {
                ...
                $storePolice = app::get('toptemai')->rpcCall('item.store.info',$storeParams);
            ...
            else
            {
                ...
                $params['store'] = $storePolice['policevalue']?$storePolice['policevalue']:0;
                $itemsList = app::get('toptemai')->rpcCall('item.store.police', $params);
            }
        ...
        {
            $itemsList = app::get('toptemai')->rpcCall('item.search',$params);
        }
    ...
    {
        return [
                'cat_name_l1' => app::get('toptemai')->_('一级类目'),
                'cat_name_l2' => app::get('toptemai')->_('二级类目'),
                'cat_name_l3' => app::get('toptemai')->_('三级类目'),
                'shop_CatName'=> app::get('toptemai')->_('店铺分类'),
                'title' => app::get('toptemai')->_('商品标题'),
                'sub_title' => app::get('toptemai')->_('商品副标题'),
                'brand_name' => app::get('toptemai')->_('品牌'),
                'bn' => app::get('toptemai')->_('商品货号'),
                'barcode' => app::get('toptemai')->_('条形码'),
                'platfrom' => app::get('toptemai')->_('发布平台'),
                'price' => app::get('toptemai')->_('销售价'),
                'realStore' => app::get('toptemai')->_('库存'),
                'sub_stock_store' => app::get('toptemai')->_('库存计数'),
                'mkt_price' => app::get('toptemai')->_('原价'),
                'is_show_mkt_price' => app::get('toptemai')->_('是否显示原价'),
                'cost_price' => app::get('toptemai')->_('成本价'),
                'weight' => app::get('toptemai')->_('重量'),
                'unit' => app::get('toptemai')->_('计价单位'),
                'dlytmpl_name' => app::get('toptemai')->_('运费模板'),
                'spec_info' => app::get('toptemai')->_('规格值'),
        ];
    }
    ...
    protected function formatExportItemList($itemList)
    {
        ...
        $itemsSku = app::get('toptemai')->rpcCall('sku.search', $skuParams);
    ...
    protected function compositeData($itemTmps, $blendData)
    {
        ...
        foreach ($itemTmps as &$val)
        {
            ...
            // 处理发布端
            $use_platform = [
                0 => app::get('toptemai')->_('全部'),
                1 => app::get('toptemai')->_('pc端'),
                2 => app::get('toptemai')->_('wap端'),
            ];
            $val['platfrom'] = $use_platform[$val['use_platform']];
            unset($val['use_platform']);
        
            // 是否显示原价
            $val['is_show_mkt_price'] = app::get('toptemai')->_('否');
            if($val['show_mkt_price'])
            {
                $val['is_show_mkt_price'] = app::get('toptemai')->_('是');
            }
    ...
    protected function getItemCatAndBrand($itemList)
    {
        ...
        $brandList = app::get('toptemai')->rpcCall('category.brand.get.list', $brandParams);
        ...
        $catList = app::get('toptemai')->rpcCall('category.cat.get.data', $catParams);
        $result['cat'] = $catList;
        if(count($catIds) < 2)
        {
            $result['cat'][] = $catList;
