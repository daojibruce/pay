...
class toptemai_item_import {
    ...
    public function import($fileInfo)
    {
        ...
        foreach ($formatData as $val)
        {
            ...
            try {
                app::get('toptemai')->rpcCall('item.create', $val);
            } catch (Exception $e) {
           ...
           foreach ($item as $hash=>$text)
           {
               if(isset($colsRule[$hash]) && $colsRule[$hash]['required'])
               {
                   $validator = validator::make(
                           ['text' => $text],
                           ['text' => 'required'],
                           ['text' => app::get('toptemai')->_('表格第'.($r+2).'行的'.$colsRule[$hash][0].'必填')]
                           );
                   $validator->newFails();
               }
           }
    ...
    protected function checkItems($items, $catId, $colsTitle)
    {
        ...
        $params['cat_id'] = $catId;
        $brand = app::get('toptemai')->rpcCall('category.get.cat.rel.brand',$params);
        // 获取商家分类
        $shopCatParams['shop_id'] = $this->shopId;
        $shopCatParams['fields'] = 'cat_id,cat_name,parent_id';
        $shopCatList = app::get('toptemai')->rpcCall('shop.cat.get', $shopCatParams);
        ...
        $dlyList = [];
        $dlyList = app::get('toptemai')->rpcCall('logistics.dlytmpl.get.list', $dlyParams);
        $dlyList = $dlyList['data'];
        // 获取销售属性
        $specProps = app::get('toptemai')->rpcCall('category.catprovalue.get',array('cat_id'=>$catId,'type'=>'spec'));
        // 获取自然属性
        $natureProps = app::get('toptemai')->rpcCall('category.catprovalue.get',array('cat_id'=>$catId,'type'=>'nature'));
        ...
        foreach ($itemsTmp as $k=>$item)
        {
            ...
            {
                $notice = app::get('toptemai')->_('商品标识符为'.$k.':%s');
            }
            else
            {
                $notice = app::get('toptemai')->_('表格第'.($k+2).'行:%s');
            }
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品标题要相同')));
            }
            ...
            if(count($subTitle) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品副标题要相同')));
            }
            ...
            if(count($weights) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品重量要相同')));
            }
            $weight = implode(',', $weights);
            if(strlen((string)$weight) > 13)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品重量格式错误')));
            }
            ...
            if(count($units) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品计价单位要相同')));
            }
            $unit = implode(',', $units);
            
            if(mb_strlen($unit, 'utf-8') > 3)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品计价单位不能超过三个字')));
            }
            $realItem['unit'] = $unit;
            ...
            if(count($brandArr) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品品牌要相同')));
            }
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品品牌填写错误')));
            }
            ...
            if(count($shopCat) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品店铺分类要相同')));
            }
            ...
            foreach ($shopName as $sv)
            {
                ...
                {
                    throw new LogicException(sprintf($notice, app::get('toptemai')->_('店铺分类填写错误')));
                }
                ...
                if(array_key_exists($parentCat, $shopParentCat))
                {
                    ...
                    {
                        throw new LogicException(sprintf($notice, app::get('toptemai')->_('店铺二级分类填写错误')));
                    }
                }
                else
                {
                    throw new LogicException(sprintf($notice, app::get('toptemai')->_('店铺一级分类填写错误')));
                }
            }
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('发布端填写错误')));
            }
            $platform = '0,1,2';
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('库存计数不一致')));
            }
            
            if(strpos($zeroOne, implode(',', $sub_stock_store)) === false)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('库存计数填写错误')));
            }
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('是否显示原价填写不一致')));
            }
            
            if(strpos($zeroOne, implode(',', $is_show_mkt_price)) === false)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('是否显示原价填写错误')));
            }
            ...
            if(count($dlyName) > 1)
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('运费模板名称不一致')));
            }
            ...
            {
                throw new LogicException(sprintf($notice, app::get('toptemai')->_('运费模板填写错误')));
            }
            ...
            if($natureProps)
            {
                ...
                foreach ($natureProps as $t)
                {
                    ...
                    if(count($natValue) > 1)
                    {
                        throw new LogicException(sprintf($notice, app::get('toptemai')->_($colsTitle[$i].'不一致')));
                    }
    ...
    protected function prosku($specProps, $item, $notice, $realItem)
    {
        ...
        {
            throw new LogicException(sprintf($notice, app::get('toptemai')->_('商品规格值错误')));
        }
        ...
        foreach ($item as $itv)
        {
            ...
            if ($itv['spec_info'] && $specProps)
            {
                ...
                foreach ($skuInput as $k=>$text)
                {
                    ...
                    {
                        throw new LogicException(sprintf($notice, app::get('toptemai')->_('此分类下没有'.$text.'这个规格')));
                    }
    ...
    protected function checkCat($data, $colCount)
    {
        ...
        {
            throw new LogicException(app::get('toptemai')->_('商品一级类目必须相同'));
        }
         
        if(count(array_unique($tmp['cat_name_l2'])) !=1)
        {
            throw new LogicException(app::get('toptemai')->_('商品二级类目必须相同'));
        }
         
        if(count(array_unique($tmp['cat_name_l3'])) !=1)
        {
            throw new LogicException(app::get('toptemai')->_('商品三级类目必须相同'));
        }
        ...
        if(!$catId)
        {
            throw new LogicException(app::get('toptemai')->_('商品三级分类错误'));
        }
        ...
        if($catCols != $colCount)
        {
            throw new LogicException(app::get('toptemai')->_('请确认三级分类模板是否正确'));
        }
    ...
    protected function readDocument($fileInfo)
    {
        ...
        {
            throw new LogicException(app::get('toptemai')->_('最多上传'.$this->allowLimit.'条商品信息'));
        }
        
        if(!$row || ($row-1) <= 0)
        {
            throw new LogicException(app::get('toptemai')->_('请填写商品信息'));
        }
    ...
    protected function checkFile($file)
    {
        ...
        {
            throw new LogicException(app::get('toptemai')->_('文件类型错误'));
        }
    ...
    protected function getColsTitle($catId)
    {
        ...
        $colsTitle['same_item'] = app::get('toptemai')->_('同一商品标识');
        $data[0] = $colsTitle;
    ...
    {
        $shopId = $this->shopId;
        $shopAuthorize = app::get('toptemai')->rpcCall('shop.authorize.catbrandids.get',array('shop_id'=>$shopId));
        $catId = $shopAuthorize[$shopId]['cat'];
        $shopType = $shopAuthorize[$shopId]['shop_type'];
        if(!$catId && $shopType == "self")
        {
            $catList = app::get('toptemai')->rpcCall('category.cat.get.list');
        }
        elseif($catId)
        {
            $catList = app::get('toptemai')->rpcCall('category.cat.get',array('cat_id'=>implode(',',$catId)));
        }
    ...
    {
        $cols = [
                'cat_name_l1' => [app::get('toptemai')->_('一级类目'), 'required'=>true],
                'cat_name_l2' => [app::get('toptemai')->_('二级类目'), 'required'=>true],
                'cat_name_l3' => [app::get('toptemai')->_('三级类目'), 'required'=>true],
                'shop_CatName'=> [app::get('toptemai')->_('店铺分类'), 'required'=>true],
                'title' => [app::get('toptemai')->_('商品标题'), 'required'=>true],
                'sub_title' => [app::get('toptemai')->_('商品副标题'), 'required'=>false],
                'brand_name' => [app::get('toptemai')->_('品牌'), 'required'=>true],
                'bn' => [app::get('toptemai')->_('商品货号'), 'required'=>false],
                'barcode' => [app::get('toptemai')->_('条形码'), 'required'=>false],
                'platfrom' => [app::get('toptemai')->_('发布平台'), 'required'=>true],
                'price' => [app::get('toptemai')->_('销售价'), 'required'=>true],
                'realStore' => [app::get('toptemai')->_('库存'), 'required'=>true],
                'sub_stock_store' => [app::get('toptemai')->_('库存计数'), 'required'=>true],
                'mkt_price' => [app::get('toptemai')->_('原价'), 'required'=>true],
                'is_show_mkt_price' => [app::get('toptemai')->_('是否显示原价'), 'required'=>true],
                'cost_price' => [app::get('toptemai')->_('成本价'), 'required'=>false],
                'weight' => [app::get('toptemai')->_('重量'), 'required'=>true],
                'unit' => [app::get('toptemai')->_('计价单位'), 'required'=>true],
                'dlytmpl_name' => [app::get('toptemai')->_('运费模板'), 'required'=>true],
                'spec_info' => [app::get('toptemai')->_('规格值'), 'required'=>false],
        ];
