 ...
 */
class sysstat_data_temai_shoptrade
{
	...
	private function __check($filter)
	{
		...
		{
			throw new \LogicException(app::get('sysstat')->_('平台展销会员id不能空!'));
		}
		return true;
	}
	 ...
	 * @return array data
	 */
	public function getTimeInfo($inforType,$type,$userId,$filter=null,$limit = -1,$start = 0,$orderBy=null)
	{
		$statFilter = $this->__getTypeFilter($type,$userId,$filter,$limit,$start,$orderBy);
		switch ($inforType) {
			case 'trade':
	...
	private function __getTypeFilter($type,$userId,$filter,$limit,$start,$orderBy)
	{
		...
		switch ($type) {
			case 'yesterday':
				return array(
						'commonday'=>array(
							'filter'=>array('user_id'=>$userId,'createtime|nequal'=>$filterTime['yesterday']),
							'limit'=>$limit,
							'start'=>$start,
							'orderBy'=>$orderBy
							),
						'beforeweek'=>array(
							'filter'=>array('user_id'=>$userId,'createtime|nequal'=>$filterTime['beforeweek']),
							'limit'=>$limit,
							'start'=>$start,
							'orderBy'=>$orderBy
							),
			...
			case 'beforday':
				return array(
						'commonday'=>array(
							'filter'=>array('user_id'=>$userId,'createtime|nequal'=>$filterTime['beforday']),
							'limit'=>$limit,
							'orderBy'=>$orderBy
							),
						'beforeweek'=>array(
			...
			case 'beforeweek':
				return array(
						'filter'=>array('user_id'=>$userId,'createtime|bthan'=>$filterTime),
						'limit'=>$limit,
						'start'=>$start,
						'orderBy'=>$orderBy
						);
			...
			case 'select':
				return array(
						'filter'=>array('user_id'=>$userId,'createtime|bthan'=>$filterTime['before']['start'],'createtime|sthan'=>$filterTime['before']['end']),
						'limit'=>$limit,
						'start'=>$start,
						'orderBy'=>$orderBy
						);
			...
			case 'selecttime':
				return array(
							...
							'after'=>array(
								'filter'=>array('user_id'=>$userId,'createtime|bthan'=>$filterTime['after']['start'],'createtime|sthan'=>$filterTime['after']['end']),
								'limit'=>$limit,
								'start'=>$start,
								'orderBy'=>$orderBy
							),
	...
	public function getStatInfo($getData,$type=null)
	{
		if($type=='yesterday' || $type=='beforday')
		{
			foreach ($getData['commonday'] as $key => $value)
			{
				...
				$commonday['user_id'] =$value['user_id'];
			...
			foreach ($getData['beforeweek'] as $key => $value)
			{
				...
				$beforeweek['user_id'] =$value['user_id'];
	...
	public function getDataInfo($newdata)
	{
		foreach ($newdata as $keys => $values)
		{
			...
			$datas[$keys]['user_id'] =end($values)['user_id'];
